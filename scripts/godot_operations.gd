...
func _init():
	...
	if error == OK:
		...
		log_error(
			"JSON Error: " + json.get_error_message() + " at line " + str(json.get_error_line())
		)
		quit(1)
	...
	for global_class in global_classes:
		...
		if found_name_of_class == name_of_class:
			if debug_mode:
				print(
					(
						"Found matching class in registry: "
						+ found_name_of_class
						+ " at path: "
						+ found_path
					)
...
func instantiate_class(name_of_class):
	...
	if ClassDB.class_exists(name_of_class):
		...
		if ClassDB.can_instantiate(name_of_class):
			...
			printerr(
				"This may be an abstract class or interface that cannot be directly instantiated"
			)
	else:
	...
	if result == null:
		...
		print(
			"Successfully instantiated class: " + name_of_class + " of type: " + result.get_class()
		)
		...
		if initial_test_file:
			...
			if initial_test_file_exists:
				var remove_error = DirAccess.remove_absolute(
					ProjectSettings.globalize_path(initial_test_file_path)
				)
			...
			if debug_mode:
				...
				if not file_check_abs and not file_check_res:
					...
					if test_scene_file:
						...
						print(
							(
								"Successfully wrote test file to scene directory: "
								+ test_scene_file_path
							)
						)
						...
						if test_file_exists:
							...
							printerr(
								"This suggests an issue with ResourceSaver.save() or the packed scene."
							)

							# Try saving with a different approach
							print("Trying alternative save approach...")
							var alt_save_error = ResourceSaver.save(
								packed_scene, test_scene_file_path + ".tscn"
							)
							print("Alternative save result: " + str(alt_save_error))

							# Clean up test files
							DirAccess.remove_absolute(
								ProjectSettings.globalize_path(test_scene_file_path)
							)
							if alt_save_error == OK:
								DirAccess.remove_absolute(
									ProjectSettings.globalize_path(test_scene_file_path + ".tscn")
								)
						else:
							printerr(
								"Test file couldn't be verified. This suggests filesystem access issues."
							)
					else:
						var write_error = FileAccess.get_open_error()
						printerr(
							"Failed to write test file to scene directory: " + str(write_error)
						)
						printerr(
							"This confirms there are permission or path issues with the scene directory."
						)
				...
				if file_check_abs or file_check_res or res_exists:
					...
					if test_load:
						...
						print(
							"Scene file exists but cannot be loaded. It may be corrupted or incomplete."
						)
...
func get_uid(params):
	...
	if f:
		...
		var result = {
			...
			"exists": false,
			"message":
			"UID file does not exist for this file. Use resave_resources to generate UIDs."
		}
		if debug_mode:
			print("UID result: " + JSON.stringify(result))
		print(JSON.stringify(result))
				...
				if error == OK:
					...
					if debug_mode:
						...
						if not uid_check_after:
							printerr(
								"UID file reported as generated but does not exist at: " + uid_path
							)
				else:
					printerr(
						"Failed to generate UID for: " + script_path + ", error: " + str(error)
					)
			else:
		...
		if prop["name"] == prop_name:
			...
			match type_id:
				...
				TYPE_VECTOR3I:
					if value is Dictionary and value.has("x") and value.has("y"):
						return Vector3i(
							int(value.get("x", 0)), int(value.get("y", 0)), int(value.get("z", 0))
						)
				TYPE_COLOR:
				...
				TYPE_AABB:
					if value is Dictionary and value.has("position") and value.has("size"):
						...
						return AABB(
							...
							Vector3(
								float(sz.get("x", 0)), float(sz.get("y", 0)), float(sz.get("z", 0))
							)
						)
				TYPE_BASIS:
					if value is Dictionary and value.has("x") and value.has("y") and value.has("z"):
						...
						return Basis(
							Vector3(
								float(bx.get("x", 0)), float(bx.get("y", 0)), float(bx.get("z", 0))
							),
							Vector3(
								float(by.get("x", 0)), float(by.get("y", 0)), float(by.get("z", 0))
							),
							Vector3(
								float(bz.get("x", 0)), float(bz.get("y", 0)), float(bz.get("z", 0))
							)
						)
				TYPE_TRANSFORM3D:
					if value is Dictionary and value.has("basis") and value.has("origin"):
						...
						var origin = Vector3(
							float(origin_d.get("x", 0)),
							float(origin_d.get("y", 0)),
							float(origin_d.get("z", 0))
						)
	...
	if not scene:
		...
		if f:
			...
			print(
				JSON.stringify(
					{
						"error": "Failed to instantiate scene, returning raw text",
						"raw": raw_content.substr(0, 4096)
					}
				)
			)
...
func modify_node(params):
	...
	for prop_name in properties:
		...
		log_info(
			"Setting " + prop_name + " = " + str(converted_value) + " (from " + str(raw_value) + ")"
		)
...
func manage_scene_signals(params):
	...
	if action == "list":
		...
		var conn_line = (
			...
			% [signal_name, source_path, target_path, method]
		)
		content += "\n" + conn_line + "\n"

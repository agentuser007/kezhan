...
func _try_connect() -> void:
	...
	if status != StreamPeerTCP.STATUS_NONE and status != StreamPeerTCP.STATUS_ERROR:
		push_warning(
			(
				"BrokerClient: _try_connect called in unexpected status %d, skipping connect_to_host"
				% status
			)
		)
		return
...
func _handle_disconnect() -> void:
	push_warning(
		"BrokerClient: connection lost to %s:%d (executor_id=%s)" % [_host, _port, _executor_id]
	)
	_connected = false
	_executor_id = ""
	_buffer = ""
...
func _send_message(msg: Dictionary) -> void:
	if _tcp.get_status() != StreamPeerTCP.STATUS_CONNECTED:
		push_warning(
			(
				"BrokerClient: _send_message called while not connected (status=%d), dropping message: %s"
				% [_tcp.get_status(), msg.get("type", "unknown")]
			)
		)
		return
	...
	if err != OK:
		push_warning(
			(
				"BrokerClient: put_data failed with error %d for message type: %s"
				% [err, msg.get("type", "unknown")]
			)
		)

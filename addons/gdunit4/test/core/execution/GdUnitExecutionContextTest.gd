...
func test_collect_report_statistics_with_errors() -> void:
	...
	if ctx_suite != null:
		var suite_err1 := ctx_suite.add_report(
			GdUnitReport.new().create(GdUnitReport.FAILURE, 1, "suite before error")
		)
		...
		if ctx_test != null:
			...
			var test_hook_err1 := ctx_test_hook.add_report(
				GdUnitReport.new().create(GdUnitReport.FAILURE, 3, "before_test error")
			)
			# test execution
			var ctx_test_call := GdUnitExecutionContext.of(ctx_test_hook)
			var test_err1 := ctx_test_call.add_report(
				GdUnitReport.new().create(GdUnitReport.FAILURE, 13, "test error_a")
			)
			var test_err2 := ctx_test_call.add_report(
				GdUnitReport.new().create(GdUnitReport.FAILURE, 14, "test error_b")
			)
			var test_err3 := ctx_test_call.add_report(
				GdUnitReport.new().create(GdUnitReport.FAILURE, 15, "test error_c")
			)
			ctx_test_call.gc(GdUnitExecutionContext.GC_ORPHANS_CHECK.TEST_CASE)

			var test_hook_err2 := ctx_test_hook.add_report(
				GdUnitReport.new().create(GdUnitReport.FAILURE, 4, "after_test error")
			)
			...
			assert_array(test_reports).contains_exactly(
				[test_hook_err1, test_hook_err2, test_err1, test_err2, test_err3]
			)
			(
				assert_dict(test_statisitcs)
				. is_equal(
					{
		...
		var suite_err2 := ctx_suite.add_report(
			GdUnitReport.new().create(GdUnitReport.FAILURE, 1, "suite after error")
		)
...
func test_collect_report_statistics_only_errors_on_test_hooks() -> void:
	...
	if ctx_suite != null:
		...
		if ctx_test != null:
			...
			var err1 := ctx_test_hook.add_report(
				GdUnitReport.new().create(GdUnitReport.FAILURE, 1, "error on before_test()")
			)
			# test execution
			var ctx_test_call := GdUnitExecutionContext.of(ctx_test_hook)
			ctx_test_call.gc(GdUnitExecutionContext.GC_ORPHANS_CHECK.TEST_CASE)
	...
	if ctx_suite != null:
		...
		for index in range(0, 10):
			...
			if ctx_test != null:
				...
				var ctx_test_call := GdUnitExecutionContext.of(ctx_test_hook)
				ctx_test_call.gc(GdUnitExecutionContext.GC_ORPHANS_CHECK.TEST_CASE)
				var expected_report := ctx_test_call.add_report(
					GdUnitReport.new().create(
						GdUnitReport.SKIPPED, index * 5, "skipped test %d" % index
					)
				)
				ctx_test_hook.gc(GdUnitExecutionContext.GC_ORPHANS_CHECK.TEST_HOOK_AFTER)
				# verify
...
func test_simmulate_flaky_test(
	retry_count: int,
	is_flaky: bool,
	is_failed: bool,
	_test_parameters := [
	...
	if ctx_suite != null:
		...
		if ctx_test != null:
				...
				if retry < 2:
					expected_reports.append(
						ctx_test_call.add_report(
							GdUnitReport.new().create(GdUnitReport.FAILURE, 42, "error")
						)
					)
					expected_reports.append(
						ctx_test_call.add_report(
							GdUnitReport.new().create(GdUnitReport.FAILURE, 43, "error")
						)
					)
				ctx_test_hook.gc(GdUnitExecutionContext.GC_ORPHANS_CHECK.TEST_HOOK_AFTER)
			...
			(
				assert_bool(statistics[GdUnitEvent.FLAKY])
				. override_failure_message("Shold be marked as %s" % "flaky" if is_flaky else "")
				. is_equal(is_flaky)
			)
			(
				assert_bool(statistics[GdUnitEvent.FAILED])
				. override_failure_message("Expect be failing: %s" % is_failed)
				. is_equal(is_failed)
			)
...
static func create_test_case(
	p_name: String, p_line_number: int, p_script_path: String
) -> _TestCase:

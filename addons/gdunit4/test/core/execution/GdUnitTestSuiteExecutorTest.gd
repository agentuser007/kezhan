...
func run_tests(tests: Array[GdUnitTestCase], settings := {}) -> Array[GdUnitEvent]:
	...
	await GdUnitThreadManager.run(
		"test_executor_%d" % randi(),
		func() -> void:
...
func assert_event_list(
	events: Array[GdUnitEvent], suite_name: String, test_case_names: Array[String]
) -> void:
	var expected_events := Array()
	expected_events.append(
		tuple(
			GdUnitEvent.TESTSUITE_BEFORE, suite_name, any_class(GdUnitGUID), test_case_names.size()
		)
	)
	...
	(
		...
		. override_failure_message(
			"Expecting be %d events emitted, but counts %d." % [expected_event_count, events.size()]
		)
...
func assert_test_counters(events: Array[GdUnitEvent]) -> GdUnitArrayAssert:
	...
	return assert_array(_events).extractv(
		extr("guid"), extr("type"), extr("error_count"), extr("failed_count"), extr("orphan_nodes")
	)
...
func assert_event_reports(events: Array[GdUnitEvent], expected_reports: Array) -> void:
	...
	for event_index in _events.size():
		...
		var expected: Array = (
			expected_reports[event_index] if expected_reports.size() > event_index else []
		)
		...
		for m in expected.size():
			if m < current.size():
				assert_str(flating_message(current[m].message() as String)).starts_with(
					str(expected[m])
				)
			else:
...
func test_execute_success() -> void:
	var tests := GdUnitTestResourceLoader.load_tests(
		"res://addons/gdUnit4/test/core/resources/testsuites/TestSuiteAllStagesSuccess.resource"
	)
	var all_tests: Array[GdUnitTestCase] = Array(
		tests.values(), TYPE_OBJECT, "RefCounted", GdUnitTestCase
	)
	...
	# all tests passses
	(
		assert_event_states(events)
		. contains_exactly(
			[
				tuple(
					any_class(GdUnitGUID),
					SUCCEEDED,
					NOT_SKIPPED,
					NO_WARNING,
					NO_FAILURES,
...
func test_execute_failure_on_stage_before() -> void:
	var tests := GdUnitTestResourceLoader.load_tests(
		"res://addons/gdUnit4/test/core/resources/testsuites/TestSuiteFailOnStageBefore.resource"
	)
	var all_tests: Array[GdUnitTestCase] = Array(
		tests.values(), TYPE_OBJECT, "RefCounted", GdUnitTestCase
	)
	...
	assert_event_reports(events, [[], [], [], [], [], ["failed on before()"]])


func test_execute_failure_on_stage_after() -> void:
	var tests := GdUnitTestResourceLoader.load_tests(
		"res://addons/gdUnit4/test/core/resources/testsuites/TestSuiteFailOnStageAfter.resource"
	)
	var all_tests: Array[GdUnitTestCase] = Array(
		tests.values(), TYPE_OBJECT, "RefCounted", GdUnitTestCase
	)
	...
	assert_event_reports(events, [[], [], [], [], [], ["failed on after()"]])
...
func test_execute_failure_on_stage_before_test() -> void:
	var tests := (
		...
		. load_tests(
			"res://addons/gdUnit4/test/core/resources/testsuites/TestSuiteFailOnStageBeforeTest.resource"
		)
	)
	var all_tests: Array[GdUnitTestCase] = Array(
		tests.values(), TYPE_OBJECT, "RefCounted", GdUnitTestCase
...
func test_execute_failure_on_stage_after_test() -> void:
	var tests := GdUnitTestResourceLoader.load_tests(
		"res://addons/gdUnit4/test/core/resources/testsuites/TestSuiteFailOnStageAfterTest.resource"
	)
	var all_tests: Array[GdUnitTestCase] = Array(
		tests.values(), TYPE_OBJECT, "RefCounted", GdUnitTestCase
	)
...
func test_execute_failure_on_stage_test_case1() -> void:
	var tests := GdUnitTestResourceLoader.load_tests(
		"res://addons/gdUnit4/test/core/resources/testsuites/TestSuiteFailOnStageTestCase1.resource"
	)
	var all_tests: Array[GdUnitTestCase] = Array(
		tests.values(), TYPE_OBJECT, "RefCounted", GdUnitTestCase
	)
...
func test_execute_failure_on_multiple_stages() -> void:
	...
	var tests := GdUnitTestResourceLoader.load_tests(
		"res://addons/gdUnit4/test/core/resources/testsuites/TestSuiteFailOnMultipeStages.resource"
	)
	var all_tests: Array[GdUnitTestCase] = Array(
		tests.values(), TYPE_OBJECT, "RefCounted", GdUnitTestCase
	)
...
func test_execute_failure_and_orphans() -> void:
	...
	var tests := (
		GdUnitTestResourceLoader
		. load_tests(
			"res://addons/gdUnit4/test/core/execution/resources/orphans/TestSuiteFailAndOrpahnsDetected.gd"
		)
	)
	var all_tests: Array[GdUnitTestCase] = Array(
		tests.values(), TYPE_OBJECT, "RefCounted", GdUnitTestCase
	...
	(
		...
		. contains_exactly(
			[
				...
				tuple(
					any_class(GdUnitGUID), SUCCEEDED, NOT_SKIPPED, WARNINGS, NO_FAILURES, NO_ERRORS
				),
			]
		)
	)
...
func test_execute_failure_and_orphans_report_orphan_disabled() -> void:
	...
	var events := await run_tests(all_tests, {GdUnitSettings.REPORT_ORPHANS: false})
...
func test_execute_error_on_test_timeout() -> void:
	...
	var tests := GdUnitTestResourceLoader.load_tests(
		"res://addons/gdUnit4/test/core/resources/testsuites/TestSuiteErrorOnTestTimeout.resource"
	)
	var all_tests: Array[GdUnitTestCase] = Array(
		tests.values(), TYPE_OBJECT, "RefCounted", GdUnitTestCase
	)
...
func test_execute_fuzzed_metrics() -> void:
	var tests := GdUnitTestResourceLoader.load_tests(
		"res://addons/gdUnit4/test/core/resources/testsuites/TestSuiteFuzzedMetricsTest.resource"
	)
	var all_tests: Array[GdUnitTestCase] = Array(
		tests.values(), TYPE_OBJECT, "RefCounted", GdUnitTestCase
	)
...
func test_execute_parameterized_metrics() -> void:
	var tests := (
		GdUnitTestResourceLoader
		. load_tests(
			"res://addons/gdUnit4/test/core/resources/testsuites/TestSuiteParameterizedMetricsTest.resource"
		)
	)
	var all_tests: Array[GdUnitTestCase] = Array(
		tests.values(), TYPE_OBJECT, "RefCounted", GdUnitTestCase
...
func test_execute_failure_fuzzer_iteration() -> void:
	# this tests a timeout on a test case reported as error
	var tests := GdUnitTestResourceLoader.load_tests(
		"res://addons/gdUnit4/test/core/resources/testsuites/GdUnitFuzzerTest.resource"
	)
	var all_tests: Array[GdUnitTestCase] = Array(
		tests.values(), TYPE_OBJECT, "RefCounted", GdUnitTestCase
	)
	...
	(
		...
		. contains_exactly(
			[
				...
				tuple(
					test_multi_yielding_with_fuzzer_fail_after_3_iterations.guid,
					GdUnitEvent.TESTCASE_BEFORE,
					0,
					0,
					0
	...
	assert_event_states(events).contains_exactly(
		[
			tuple(
				any_class(GdUnitGUID), SUCCEEDED, NOT_SKIPPED, NO_WARNING, NO_FAILURES, NO_ERRORS
			),
			tuple(
				test_multi_yielding_with_fuzzer.guid,
				SUCCEEDED,
				NOT_SKIPPED,
				NO_WARNING,
				NO_FAILURES,
...
func test_execute_add_child_on_before_GD_106() -> void:
	var tests := (
		GdUnitTestResourceLoader
		. load_tests(
			"res://addons/gdUnit4/test/core/resources/testsuites/TestSuiteFailAddChildStageBefore.resource"
		)
	)
	var all_tests: Array[GdUnitTestCase] = Array(
		tests.values(), TYPE_OBJECT, "RefCounted", GdUnitTestCase
...
func test_execute_parameterizied_tests() -> void:
	# this is a more complex failure state, we expect to find multipe failures on different stages
	var tests := GdUnitTestResourceLoader.load_tests(
		"res://addons/gdUnit4/test/core/resources/testsuites/TestSuiteParameterizedTests.resource"
	)
	var all_tests: Array[GdUnitTestCase] = Array(
		tests.values(), TYPE_OBJECT, "RefCounted", GdUnitTestCase
	)
	...
	var original_mode: Variant = ProjectSettings.get_setting(
		GdUnitSettings.REPORT_ASSERT_STRICT_NUMBER_TYPE_COMPARE
	)
	ProjectSettings.set_setting(GdUnitSettings.REPORT_ASSERT_STRICT_NUMBER_TYPE_COMPARE, true)
	var events := await run_tests(
		all_tests, {GdUnitSettings.REPORT_ASSERT_STRICT_NUMBER_TYPE_COMPARE: true}
	)
	...
	(
		...
		. contains(
			[
				...
				tuple(
					GdUnitEvent.TESTCASE_AFTER, test_1.guid, NO_WARNING, NO_ERRORS, NO_FAILURES, 0
				),
				tuple(GdUnitEvent.TESTCASE_AFTER, test_2.guid, NO_WARNING, NO_ERRORS, FAILURES, 0),
				tuple(
					GdUnitEvent.TESTCASE_AFTER, test_3.guid, NO_WARNING, NO_ERRORS, NO_FAILURES, 0
				)
			]
		)
	)
	...
	ProjectSettings.set_setting(
		GdUnitSettings.REPORT_ASSERT_STRICT_NUMBER_TYPE_COMPARE, original_mode
	)
	...
	(
		...
		. contains(
			[
				tuple(
					GdUnitEvent.TESTCASE_AFTER, test_0.guid, NO_WARNING, NO_ERRORS, NO_FAILURES, 0
				),
				tuple(
					GdUnitEvent.TESTCASE_AFTER, test_1.guid, NO_WARNING, NO_ERRORS, NO_FAILURES, 0
				),
				tuple(
					GdUnitEvent.TESTCASE_AFTER, test_2.guid, NO_WARNING, NO_ERRORS, NO_FAILURES, 0
				),
				tuple(
					GdUnitEvent.TESTCASE_AFTER, test_3.guid, NO_WARNING, NO_ERRORS, NO_FAILURES, 0
				)
...
func test_execute_test_suite_is_skipped() -> void:
	var tests := GdUnitTestResourceLoader.load_tests(
		"res://addons/gdUnit4/test/core/resources/testsuites/TestSuiteSkipped.resource"
	)
	var all_tests: Array[GdUnitTestCase] = Array(
		tests.values(), TYPE_OBJECT, "RefCounted", GdUnitTestCase
	)
	...
	(
		...
		. contains_exactly(
			[
				...
				tuple(
					any_class(GdUnitGUID), SUCCEEDED, SKIPPED, NO_WARNING, NO_FAILURES, NO_ERRORS
				),
			]
		)
	)
...
func test_execute_test_case_is_skipped() -> void:
	var tests := GdUnitTestResourceLoader.load_tests(
		"res://addons/gdUnit4/test/core/resources/testsuites/TestCaseSkipped.resource"
	)
	var all_tests: Array[GdUnitTestCase] = Array(
		tests.values(), TYPE_OBJECT, "RefCounted", GdUnitTestCase
	)
...
func test_execute_test_case_is_flaky_and_failed() -> void:
	var tests := GdUnitTestResourceLoader.load_tests(
		"res://addons/gdUnit4/test/core/resources/testsuites/TestCaseFlaky.resource"
	)
	var all_tests: Array[GdUnitTestCase] = Array(
		tests.values(), TYPE_OBJECT, "RefCounted", GdUnitTestCase
	)
	...
	var events := await run_tests(
		all_tests, {GdUnitSettings.TEST_FLAKY_CHECK: true, GdUnitSettings.TEST_FLAKY_MAX_RETRIES: 5}
	)

	(
		assert_array(events)
		. extractv(extr("type"), extr("guid"), extr("is_flaky"), extr("is_success"))
		. contains(
			[
				...
				tuple(
					GdUnitEvent.TESTCASE_AFTER, test_parameterized_flaky0.guid, !FLAKY, SUCCEEDED
				),
				tuple(
					GdUnitEvent.TESTCASE_AFTER, test_parameterized_flaky1.guid, !FLAKY, SUCCEEDED
				),
				tuple(GdUnitEvent.TESTCASE_AFTER, test_parameterized_flaky2.guid, !FLAKY, FAILED),
				tuple(
					GdUnitEvent.TESTCASE_AFTER, test_parameterized_flaky3.guid, !FLAKY, SUCCEEDED
				),
...
func test_execute_test_case_is_flaky_and_success() -> void:
	...
	# simulate flaky test suite execution
	var events := await run_tests(
		all_tests, {GdUnitSettings.TEST_FLAKY_CHECK: true, GdUnitSettings.TEST_FLAKY_MAX_RETRIES: 6}
	)
...
func test_execute_push_error_monitoring_disabled() -> void:
	var tests := (
		GdUnitTestResourceLoader
		. load_tests(
			"res://addons/gdUnit4/test/core/execution/resources/error_monitoring/TestSuiteWithPushErrorTests.gd"
		)
	)
	var all_tests: Array[GdUnitTestCase] = Array(
		tests.values(), TYPE_OBJECT, "RefCounted", GdUnitTestCase
	...
	# execute all tests
	var events := await run_tests(all_tests, {GdUnitSettings.REPORT_PUSH_ERRORS: false})

	(
		assert_array(events)
		. extractv(extr("type"), extr("test_name"), extr("is_success"))
		. contains(
			[
...
func test_execute_push_error_monitoring_enabled() -> void:
	...
	# execute all tests
	var events := await run_tests(all_tests, {GdUnitSettings.REPORT_PUSH_ERRORS: true})

...
func test_restore_restores_scalar_setting() -> void:
	...
	(
		assert_int(
			ProjectSettings.get_setting(GdUnitSettings.GDSCRIPT_WARNINGS_INFERRED_DECLARATION)
		)
		. is_equal(IGNORE)
	)
...
func test_restore_restores_dictionary_setting() -> void:
	...
	ProjectSettings.set_setting(
		GdUnitSettings.GDSCRIPT_WARNINGS_DIRECTORY_RULES, original_rules.duplicate()
	)
	snapshot.save()
	# Act
	ProjectSettings.set_setting(
		GdUnitSettings.GDSCRIPT_WARNINGS_DIRECTORY_RULES, {"res://modified": INCLUDE}
	)
	snapshot.restore()
	# Verify
	(

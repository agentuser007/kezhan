	...
	(
		assert_bool(
			GdUnitFileAccess.copy_directory(
				"res://addons/gdUnit4/test/core/resources/copy_test/folder_a/", temp_dir
			)
		)
		. is_true()
	)
...
func test_copy_directory_recursive() -> void:
	...
	(
		assert_bool(
			GdUnitFileAccess.copy_directory(
				"res://addons/gdUnit4/test/core/resources/copy_test/", temp_dir, true
			)
		)
		. is_true()
	)
...
func test_make_qualified_path() -> void:
	...
	assert_str(GdUnitFileAccess.make_qualified_path("/foo/bar/MyTest.gd")).is_equal(
		"res://foo/bar/MyTest.gd"
	)
	assert_str(GdUnitFileAccess.make_qualified_path("res://MyTest.gd")).is_equal("res://MyTest.gd")
	assert_str(GdUnitFileAccess.make_qualified_path("res://foo/bar/MyTest.gd")).is_equal(
		"res://foo/bar/MyTest.gd"
	)
...
func test_delete_path_index_lower_equals_than() -> void:
	...
	(
		assert_int(GdUnitFileAccess.delete_path_index_lower_equals_than(temp_dir, "report_", 0))
		. is_equal(0)
	)
	...
	(
		assert_int(GdUnitFileAccess.delete_path_index_lower_equals_than(temp_dir, "report_", 30))
		. is_equal(21)
	)
...
func test_scan_dir() -> void:
	...
	assert_array(GdUnitFileAccess.scan_dir(temp_dir)).has_size(6).contains(
		["report_2", "report_4", "report_5", "report_6", "file_a", "file_b"]
	)
...
func test_delete_directory() -> void:
	...
	assert_array(GdUnitFileAccess.scan_dir(tmp_dir)).contains_exactly_in_any_order(
		["data1", "data2", "example_a.txt", "example_b.txt", ".hidden_file.txt"]
	)
...
func test_delete_directory_content_only() -> void:
	...
	assert_array(GdUnitFileAccess.scan_dir(tmp_dir)).contains_exactly_in_any_order(
		["data1", "data2", "example_a.txt", "example_b.txt"]
	)
...
func test_extract_package() -> void:
	...
	(
		assert_array(GdUnitFileAccess.scan_dir(tmp_path))
...
func test_extract_package_invalid_package() -> void:
	...
	# now extract to temp
	var result := GdUnitFileAccess.extract_zip(source, tmp_path)
	assert_result(result).is_error().contains_message(
		(
			"Extracting `%s` failed! Please collect the error log and report this. Error Code: 1"
			% source
		)
	)
	assert_array(GdUnitFileAccess.scan_dir(tmp_path)).is_empty()

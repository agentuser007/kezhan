...
func test_load_fail() -> void:
	...
	assert_result(config.load_config("invalid_path")).is_warning().contains_message(
		"Can't find test runner configuration 'invalid_path'! Please select a test to run."
	)
...
func test_save_load() -> void:
	...
	var test_to_save: Array[GdUnitTestCase] = [
		GdUnitTestCase.from(
			"res://test/example_suite.gd", "res://test/example_suite.gd", 10, "test_a"
		),
		GdUnitTestCase.from(
			"res://test/example_suite.gd", "res://test/example_suite.gd", 14, "test_b"
		),
		GdUnitTestCase.from(
			"res://test/example_suite.gd", "res://test/example_suite.gd", 16, "test_c"
		)
	]

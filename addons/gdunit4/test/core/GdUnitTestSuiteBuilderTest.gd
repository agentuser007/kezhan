...
func before_test() -> void:
	...
	var result := GdUnitFileAccess.copy_file(
		"res://addons/gdUnit4/test/core/resources/sources/test_person.gd", temp
	)
...
func test_create_gd_success() -> void:
	...
	@warning_ignore("unsafe_cast")
	assert_tests(load(info.get("path") as String) as Script).contains_exactly_in_any_order(
		["test_first_name", "test_fully_name"]
	)

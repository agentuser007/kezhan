...
const DiscoverExampleTestSuite: GDScript = preload(
...
func test_is_skipped_entire_suite_by_full_path() -> void:
	...
	GdUnitTestDiscoverer.discover_tests(
		DiscoverExampleTestSuite, func(test: GdUnitTestCase) -> void: tests.append(test)
	)
	...
	for test in tests:
		(
			assert_bool(runner.is_skipped(test))
			. override_failure_message("Expect '%s' is skipped" % test.test_name)
			. is_true()
		)
...
func test_is_skipped_single_test_by_full_path() -> void:
	...
	runner.skip_test_suite(
		"res://addons/gdUnit4/test/core/discovery/resources/DiscoverExampleTestSuite.gd:test_case1"
	)
	...
	for test in tests:
		if test.test_name in ["test_case1", "test_case2"]:
			...
			(
				assert_bool(runner.is_skipped(test))
				. override_failure_message("Expect '%s' is NOT skipped" % test.test_name)
				. is_false()
			)

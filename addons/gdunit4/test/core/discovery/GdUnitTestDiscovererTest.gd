...
func test_discover_tests() -> void:
	...
	var discovered_tests := []
	GdUnitTestDiscoverer.discover_tests(
		script, func discover(test_case: GdUnitTestCase) -> void: discovered_tests.append(test_case)
	)
	...
	(
		...
		. contains_exactly(
			[
				...
				tuple("test_parameterized_static", "test_parameterized_static:2 (3, 3)"),
				tuple(
					"test_parameterized_static_external",
					"test_parameterized_static_external:0 (<null>)"
				),
				tuple(
					"test_parameterized_static_external",
					"test_parameterized_static_external:1 (%s)" % Vector2.ONE
				),
				tuple(
					"test_parameterized_static_external",
					"test_parameterized_static_external:2 (%s)" % Vector2i.ONE
				),
				tuple("test_parameterized_dynamic", "test_parameterized_dynamic:0 (<null>)"),
				tuple(
					"test_parameterized_dynamic", "test_parameterized_dynamic:1 (%s)" % Vector2.ONE
				),
				tuple(
					"test_parameterized_dynamic", "test_parameterized_dynamic:2 (%s)" % Vector2i.ONE
				),
		...
		. contains_exactly(
			[
				tuple(
					"test_foo3",
					"test_foo3",
					"addons.gdUnit4.test.core.resources.scan_testsuite_inheritance.by_class_name.ExtendsExtendedTest.test_foo3"
				),
				tuple(
					"test_foo2",
					"test_foo2",
					"addons.gdUnit4.test.core.resources.scan_testsuite_inheritance.by_class_name.ExtendsExtendedTest.test_foo2"
				),
				tuple(
					"test_foo1",
					"test_foo1",
					"addons.gdUnit4.test.core.resources.scan_testsuite_inheritance.by_class_name.ExtendsExtendedTest.test_foo1"
				)
			]
		)
	)
...
func test_discover_csharp_tests(_do_skip := !GdUnit4CSharpApiLoader.is_api_loaded()) -> void:
	var script: Script = load(
		"res://addons/gdUnit4/test/core/discovery/resources/DiscoverExampleTestSuite.cs"
	)
	var discovered_tests := []
	GdUnitTestDiscoverer.discover_tests(
		script, func discover(test_case: GdUnitTestCase) -> void: discovered_tests.append(test_case)
		...
		. contains_exactly(
			[
				tuple(
					"TestCase1",
					"TestCase1",
					"gdUnit4.addons.gdUnit4.test.core.discovery.resources.DiscoverExampleTestSuite.TestCase1"
				),
				tuple(
					"TestCase2",
					"TestCase2",
					"gdUnit4.addons.gdUnit4.test.core.discovery.resources.DiscoverExampleTestSuite.TestCase2"
				)
			]
		)
	)
...
func test_scan_test_directories() -> void:
	...
	(
		assert_array(GdUnitTestDiscoverer.scan_test_directories("res://", "res://", []))
		. contains_exactly(["res://addons", "res://assets"])
	)
...
func test_scan_all_test_directories() -> void:
	...
	assert_array(GdUnitTestDiscoverer.scan_all_test_directories("res://")).contains_exactly(
		["res://"]
	)

	# Test when test_root_folder is set to a specific folder
	assert_array(GdUnitTestDiscoverer.scan_all_test_directories("test")).contains_exactly(
		["res://addons/gdUnit4/test"]
	)

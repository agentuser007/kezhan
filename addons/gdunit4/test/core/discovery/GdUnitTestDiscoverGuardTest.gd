...
func test_sync_cache() -> void:
	# setup example tests
	var test1 := GdUnitTestCase.from(
		"res://test/my_test_suite.gd", "res://test/my_test_suite.gd", 23, "test_a"
	)
	var test2 := GdUnitTestCase.from(
		"res://test/my_test_suite.gd", "res://test/my_test_suite.gd", 42, "test_b"
	)
	...
	assert_dict(discoverer._discover_cache).contains_key_value(
		"res://test/my_test_suite.gd", [test1, test2]
	)
...
func test_discover_new_suite_GDScript() -> void:
	...
	await discoverer.discover(
		script,
		func(test_case: GdUnitTestCase) -> void:
	...
	(
		...
		. extractv(extr("test_name"), extr("attribute_index"))
		. contains_exactly(
			[
				tuple("test_case1", -1),
				tuple("test_case2", -1),
	...
	assert_dict(discoverer._discover_cache).contains_key_value(
		script.resource_path, discovered_tests
	)
...
func test_discover_deleted_test_GDScript() -> void:
	...
	script.source_code = script.source_code.replace("test_case1", "_test_case1").replace(
		"test_case2", "_test_case2"
	)
...
func test_discover_renamed_test_GDScript() -> void:
	...
	# we simmulate deleted tests
	script.source_code = script.source_code.replace("test_case1", "test_case11").replace(
		"test_case2", "test_foo"
	)
	assert_int(script.reload(true)).is_equal(OK)
...
func test_discover_moved_test_GDScript() -> void:
	...
	await discoverer.discover(
		...
		func(test_case: GdUnitTestCase) -> void:
			...
			if (
				test_case.test_name
				in ["test_parameterized_static_external", "test_parameterized_dynamic"]
			):
				expected_renamed_tests.append(test_case)
	)
	# verify the expected tests are collected (2 test each with a dataset of 3 == 6)
	(
		assert_array(expected_renamed_tests)
		. extractv(extr("test_name"), extr("attribute_index"), extr("line_number"))
		. contains_exactly(
			[

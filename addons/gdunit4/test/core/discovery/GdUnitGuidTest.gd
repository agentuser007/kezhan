...
func test_performance() -> void:
	...
	for n in construction_count:
		GdUnitGUID.new()
	var error_message := (
		"Expected to construct %d `GdUnitGUID` instances in less than 50ms but took %s ms"
		% [construction_count, time.elapsed_since_ms()]
	)
	assert_int(time.elapsed_since_ms()).override_failure_message(error_message).is_less(50)
	prints(
		(
			"Construction of %d 'GdUnitGUID's tooks %d ms"
			% [construction_count, time.elapsed_since_ms()]
		)
	)

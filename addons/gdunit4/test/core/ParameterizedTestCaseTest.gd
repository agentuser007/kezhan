...
var _expected_tests := {
	"test_parameterized_bool_value": [[0, false], [1, true]],
	"test_parameterized_int_values": [[1, 2, 3, 6], [3, 4, 5, 12], [6, 7, 8, 21]],
	"test_parameterized_float_values": [[2.2, 2.2, 4.4], [2.2, 2.3, 4.5], [3.3, 2.2, 5.5]],
	"test_parameterized_string_values":
	[["2.2", "2.2", "2.22.2"], ["foo", "bar", "foobar"], ["a", "b", "ab"]],
	...
	"test_parameterized_obj_values": [[TestObj.new("abc"), TestObj.new("def"), "abcdef"]],
	"test_parameterized_dict_values":
	[[{"key_a": "value_a"}, '{"key_a":"value_a"}'], [{"key_b": "value_b"}, '{"key_b":"value_b"}']],
	"test_parameterized_untyped_array": [[[42]]],
	"test_parameterized_typed_array": [[[42]]],
	"test_with_dynamic_paramater_resolving": [["test_a"], ["test_b"], ["test_c"], ["test_d"]],
	"test_with_dynamic_paramater_resolving2": [["test_a"], ["test_b"], ["test_c"]],
	"test_with_extern_parameter_set": [["test_a"], ["test_b"], ["test_c"]],
	"test_with_extern_const_parameter_set": [["aa"], ["bb"]]
}
		...
		if _collected_tests.has(test_name):
			...
			(
				...
				. override_failure_message(
					(
						"Expecting '%s' called with parameters:\n %s\n but was\n %s"
						% [test_name, expected_values, current_values]
					)
				)
...
func test_parameterized_bool_value(
	a: int, expected: bool, _test_parameters := [[0, false], [1, true]]
) -> void:
...
func test_parameterized_Vector3_values(
	a: Vector3,
	b: Vector3,
	expected: Vector3,
	_test_parameters := [
...
func test_parameterized_obj_values(
	a: Object,
	b: Object,
	expected: String,
	_test_parameters := [[TestObj.new("abc"), TestObj.new("def"), "abcdef"]]
) -> void:
	collect_test_call("test_parameterized_obj_values", [a, b, expected])
	assert_that(a.to_string() + b.to_string()).is_equal(expected)


func test_parameterized_dict_values(
	data: Dictionary,
	expected: String,
	_test_parameters := [
		[{"key_a": "value_a"}, '{"key_a":"value_a"}'], [{"key_b": "value_b"}, '{"key_b":"value_b"}']
...
func test_parameterized_untyped_array(items: Array, _test_parameters := [[[42]]]) -> void:
	collect_test_call("test_parameterized_untyped_array", [items])
	assert_array(items).contains_exactly([42])


func test_parameterized_typed_array(items: Array[int], _test_parameters := [[[42]]]) -> void:
...
func test_with_string_contains_brackets(
	...
	match test_index:
		1:
			assert_str(value).is_equal("flowchart TD\nid>This is a flag shaped node]")
		2:
			assert_str(value).is_equal("flowchart TD\nid(((This is a double circle node)))")
		3:
			assert_str(value).is_equal("flowchart TD\nid((This is a circular node))")
...
func test_with_dynamic_parameter_resolving(
	name_: String,
	value: Variant,
	expected: Variant,
	_test_parameters := [
...
func test_with_dynamic_parameter_resolving2(
	...
	_test_parameters := [
		["test_a", null, "LOG", {}],
		["test_b", Node2D, null, {Node2D: "ERROR"}],
...
var _test_set := [["test_a"], ["test_b"], ["test_c"]]
...
func test_with_extern_const_parameter_set(
	value: String, _test_parameters := [_data1, _data2]
) -> void:
	assert_str(value).is_not_empty()
	assert_array(_test_parameters).is_empty()

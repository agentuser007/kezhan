...
func test_extract_from_return_types(
	func_name: String,
	expected_type: int,
	_test_parameters := [
	...
	var method_descriptor := GdScriptTestHelper.get_script_method_descriptor(
		_return_type_variants_script, func_name
	)
...
func test_extract_from_inferred_void_pass() -> void:
	var method_descriptor := GdScriptTestHelper.get_script_method_descriptor(
		_return_type_variants_script, "inferred_void_pass"
	)
	var fd := GdFunctionDescriptor.extract_from(method_descriptor, false)
	var version := Engine.get_version_info()
	var expected_type: int = (
		GdObjects.TYPE_VARIANT if version["minor"] >= 7 else GdObjects.TYPE_VOID
	)
	assert_int(fd.return_type()).is_equal(expected_type)

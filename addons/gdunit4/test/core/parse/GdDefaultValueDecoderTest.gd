	...
	for type_id in TYPE_MAX:
		...
		(
			...
			. override_failure_message(
				"Missing Variant type '%s'" % GdObjects.type_as_string(type_id)
			)
			. is_not_null()
		)


func test_decode_Primitives(
	variant_type: int,
	value: Variant,
	expected: String,
	_test_parameters := [
...
func test_decode_Rect2(
	...
	_test_parameters := [
		...
		[
			TYPE_RECT2,
			Rect2(1, 2, 10, 20),
			"Rect2(Vector2" + str(Vector2(1, 2)) + ", Vector2" + str(Vector2(10, 20)) + ")"
		],
...
func test_decode_Transforms(
	variant_type: int,
	value: Variant,
	expected: String,
	_test_parameters := [
		[TYPE_TRANSFORM2D, Transform2D(), "Transform2D()"],
		[
			TYPE_TRANSFORM2D,
			Transform2D(2.0, Vector2(1, 2)),
			(
				"Transform2D(Vector2(-0.416147, 0.909297), Vector2(-0.909297, -0.416147), Vector2"
				+ str(Vector2(1, 2))
				+ ")"
			)
		],
		[
			TYPE_TRANSFORM2D,
			Transform2D(2.0, Vector2(1, 2), 2.0, Vector2(3, 4)),
			(
				"Transform2D(Vector2(-0.416147, 0.909297), Vector2(1.513605, -1.307287), Vector2"
				+ str(Vector2(3, 4))
				+ ")"
			)
		],
		[
			TYPE_TRANSFORM2D,
			Transform2D(Vector2(1, 2), Vector2(3, 4), Vector2.ONE),
			(
				"Transform2D(Vector2"
				+ str(Vector2(1, 2))
				+ ", Vector2"
		...
		[TYPE_TRANSFORM3D, Transform3D(), "Transform3D()"],
		[
			TYPE_TRANSFORM3D,
			Transform3D(Basis.FLIP_X, Vector3.ONE),
			(
				"Transform3D(Vector3"
				+ str(Vector3(-1, 0, 0))
				+ ", Vector3"
		...
		[
			TYPE_TRANSFORM3D,
			Transform3D(Vector3(1, 2, 3), Vector3(4, 5, 6), Vector3(7, 8, 9), Vector3.ONE),
			(
				"Transform3D(Vector3"
				+ str(Vector3(1, 2, 3))
				+ ", Vector3"
		...
		[
			TYPE_PROJECTION,
			Projection(Vector4.ONE, Vector4.ONE * 2, Vector4.ONE * 3, Vector4.ZERO),
			(
				"Projection(Vector4%s, Vector4%s, Vector4%s, Vector4%s)"
...
func test_decode_AABB(
	...
	_test_parameters := [
		...
		[
			TYPE_AABB,
			AABB(Vector3.ONE, Vector3(10, 20, 30)),
			"AABB(Vector3" + str(Vector3(1, 1, 1)) + ", Vector3" + str(Vector3(10, 20, 30)) + ")"
		],
	]
) -> void:
	assert_that(GdDefaultValueDecoder.decode_typed(variant_type, value)).is_equal(expected)
...
func test_decode_typedArrays(
	...
	expected: String,
	_test_parameters := [
		[TYPE_PACKED_BYTE_ARRAY, PackedByteArray(), "PackedByteArray()"],
		[TYPE_PACKED_BYTE_ARRAY, PackedByteArray([1, 2, 3]), "PackedByteArray([1, 2, 3])"],
		[TYPE_PACKED_COLOR_ARRAY, PackedColorArray(), "PackedColorArray()"],
		[
		...
		[TYPE_PACKED_FLOAT32_ARRAY, PackedFloat32Array(), "PackedFloat32Array()"],
		[
			TYPE_PACKED_FLOAT32_ARRAY,
			PackedFloat32Array([1.2, 2.3]),
			"PackedFloat32Array([1.20000004768372, 2.29999995231628])"
		],
		[TYPE_PACKED_FLOAT64_ARRAY, PackedFloat64Array(), "PackedFloat64Array()"],
		[
			TYPE_PACKED_FLOAT64_ARRAY,
			PackedFloat64Array([1.2, 2.3]),
			"PackedFloat64Array([1.2, 2.3])"
		],
		[TYPE_PACKED_INT32_ARRAY, PackedInt32Array(), "PackedInt32Array()"],
		[TYPE_PACKED_INT32_ARRAY, PackedInt32Array([1, 2]), "PackedInt32Array([1, 2])"],
		[TYPE_PACKED_INT64_ARRAY, PackedInt64Array(), "PackedInt64Array()"],
		[TYPE_PACKED_INT64_ARRAY, PackedInt64Array([1, 2]), "PackedInt64Array([1, 2])"],
		[TYPE_PACKED_STRING_ARRAY, PackedStringArray(), "PackedStringArray()"],
		[
			TYPE_PACKED_STRING_ARRAY,
			PackedStringArray(["aa", "bb"]),
			'PackedStringArray(["aa", "bb"])'
		],
		[TYPE_PACKED_VECTOR2_ARRAY, PackedVector2Array(), "PackedVector2Array()"],
		[
			TYPE_PACKED_VECTOR2_ARRAY,
			PackedVector2Array([Vector2.ONE, Vector2.ONE * 2]),
			(
		...
		[TYPE_PACKED_VECTOR3_ARRAY, PackedVector3Array(), "PackedVector3Array()"],
		[
			TYPE_PACKED_VECTOR3_ARRAY,
			PackedVector3Array([Vector3.ONE, Vector3.ONE * 2]),
			(
		...
		[TYPE_PACKED_VECTOR4_ARRAY, PackedVector4Array(), "PackedVector4Array()"],
		[
			TYPE_PACKED_VECTOR4_ARRAY,
			PackedVector4Array([Vector4.ONE, Vector4.ONE * 2]),
			(

...
func test_example_b(
	_a: Vector2,
	_b: Vector2,
	_test_parameters := [[Vector2.ZERO, Vector2.ONE], [Vector2(1.1, 3.2), Vector2.DOWN]]
) -> void:
	pass
...
func test_example_c(
	_a: Object,
	_b: Object,
	_test_parameters := [[Resource.new(), Resource.new()], [Resource.new(), null]]
) -> void:
	pass


func test_resolve_parameters_static(
	_a: int, _b: int, _test_parameters := [[1, 10], [2, 20]]
) -> void:
	pass


func test_resolve_parameters_at_runtime(
	_a: int, _b: int, _test_parameters := [[1, _test_param1], [2, _test_param2], [3, 30]]
) -> void:
	pass


func test_parameterized_with_comments(
	_a: int,
	_b: int,
	_c: String,
	_expected: int,
...
func test_example_d(
	_a: Vector3,
	_b: Vector3,
	_test_parameters := [[build_param(1), build_param(3)], [Vector3.BACK, Vector3.UP]]
) -> void:
	pass
...
func test_load_parameter_sets() -> void:
	assert_array(load_parameter_sets("test_example_a")).is_equal([[1, 2], [3, 4]])

	assert_array(load_parameter_sets("test_example_b")).is_equal(
		[[Vector2.ZERO, Vector2.ONE], [Vector2(1.1, 3.2), Vector2.DOWN]]
	)

	assert_array(load_parameter_sets("test_example_c")).is_equal(
		[[Resource.new(), Resource.new()], [Resource.new(), null]]
	)

	assert_array(load_parameter_sets("test_example_d")).is_equal(
		[[Vector3(1, 1, 1), Vector3(3, 3, 3)], [Vector3.BACK, Vector3.UP]]
	)

	assert_array(load_parameter_sets("test_example_e")).is_equal(
...
func test_load_parameter_with_comments() -> void:
	...
	assert_array(params).is_equal(
		[[1, 2, "3", 6], [3, 4, "5", 11], [6, 7, "string #ABCD", 21], [6, 7, "string #ABCD", 21]]
	)
...
func test_validate_test_parameter_set() -> void:
	var test_suite: GdUnitTestSuite = auto_free(
		(
			...
			. load_test_suite(
				"res://addons/gdUnit4/test/core/resources/testsuites/TestSuiteInvalidParameterizedTests.resource"
			)
		)
	)
	...
	(
		assert_is_skipped(test_suite, "test_parameterized_to_less_args", 0)
		. is_equal(
			(
				"""
	...
	(
		assert_is_skipped(test_suite, "test_parameterized_to_less_args", 1)
		. is_equal(
			(
				"""
	...
	(
		assert_is_skipped(test_suite, "test_parameterized_to_many_args", 0)
		. is_equal(
			(
				"""
			The test data set at index (0) does not match the expected test arguments:
	...
	(
		assert_is_skipped(test_suite, "test_parameterized_invalid_struct", 1)
		. is_equal(
			(
				"""
			The test data set at index (1) does not match the expected test arguments:
	...
	(
		assert_is_skipped(test_suite, "test_parameterized_invalid_args", 1)
		. is_equal(
			(
				"""
...
func assert_is_skipped(
	test_suite: GdUnitTestSuite, test_case: String, index := -1
) -> GdUnitStringAssert:
	var test := GdUnitTools.find_test_case(test_suite, test_case, index)
	if test.is_parameterized():
		# to load parameter set and force validate

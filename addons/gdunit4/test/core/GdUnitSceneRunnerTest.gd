...
func load_test_scene() -> Node:
	...
	return auto_free(
		load("res://addons/gdUnit4/test/mocker/resources/scenes/TestScene.tscn").instantiate()
	)
...
func test_get_property() -> void:
	...
	assert_that(runner.get_property("_invalid")).is_equal(
		"The property '_invalid' not exist checked loaded scene."
	)
...
func test_invoke_method() -> void:
	...
	assert_that(runner.invoke("sub", 10, 12)).is_equal(
		"The method 'sub' not exist checked loaded scene."
	)
...
func test_await_signal_without_time_factor() -> void:
	...
	(
		...
		. has_message(
			(
				"await_signal_on(panel_color_change, [%s, %s]) timed out after 300ms"
				% [str(box1), str(Color.KHAKI)]
			)
		)
		. has_line(207)
	)
...
func test_await_signal_with_time_factor() -> void:
	...
	(
		...
		. has_message(
			(
				"await_signal_on(panel_color_change, [%s, %s]) timed out after 30ms"
				% [str(box1), str(Color.KHAKI)]
			)
		)
		. has_line(224)
...
func test_runner_by_invalid_resource_path() -> void:
	...
	(
		assert_object(
			(
				scene_runner("res://addons/gdUnit4/test/core/resources/scenes/simple_scene.gd")
				. _current_scene
			)
		)
		. is_null()
...
func test_runner_by_scene_instance() -> void:
	var scene: Node = (
		load("res://addons/gdUnit4/test/core/resources/scenes/simple_scene.tscn").instantiate()
	)
...
func test_mouse_drag_and_drop() -> void:
	var spy_scene: Variant = spy(
		"res://addons/gdUnit4/test/core/resources/scenes/drag_and_drop/DragAndDropTestScene.tscn"
	)
...
func test_move_window_to_foreground() -> void:
	...
	if not Engine.is_embedded_in_editor():
		...
		var expecetd_mode := (
			...
			else DisplayServer.WINDOW_MODE_MINIMIZED
		)

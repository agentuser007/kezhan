...
class InnerTestNodeClass:
	extends Node
	pass


class InnerTestRefCountedClass:
	extends RefCounted
	pass
...
func test_richtext_normalize() -> void:
	...
	assert_that(GdUnitTools.richtext_normalize("This is a Color Message")).is_equal(
		"This is a Color Message"
	)
	...
	var message := """
		...
		"""
	(
		assert_that(GdUnitTools.richtext_normalize(message))
		. is_equal(
			"""
		line 11: Expecting:
			must be empty but was

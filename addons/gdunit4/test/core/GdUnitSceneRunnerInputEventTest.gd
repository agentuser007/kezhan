...
func assert_inital_mouse_state() -> void:
	...
	(
		...
		. override_failure_message(
			"Expecting mouse button mask %s is '0'!" % Input.get_mouse_button_mask()
		)
		. is_equal(0)
	)
...
func test_simulate_action_press() -> void:
	...
	for action in actions_to_simmulate:
		...
		(
			...
			. override_failure_message("Expect the action '%s' is pressed" % action)
			. is_true()
		)
		assert_float(Input.get_action_strength(action)).is_equal_approx(1.0, 0.1)
		assert_float(_runner.get_property("_last_pressed_strength")).is_equal_approx(1.0, 0.1)

	# other actions are not pressed
	for action: String in ["ui_accept", "ui_select", "ui_cancel"]:
		(
			assert_bool(Input.is_action_pressed(action))
			. override_failure_message("Expect the action '%s' is NOT pressed" % action)
			. is_false()
		)
...
func test_simulate_action_release() -> void:
	...
	for action in actions_to_simmulate:
		...
		(
			...
			. override_failure_message("Expect the action '%s' is released" % action)
			. is_true()
		)
	# other actions are not pressed
	for action: String in ["ui_accept", "ui_select", "ui_cancel"]:
...
func test_simulate_keypressed_as_action() -> void:
	...
	var runner := scene_runner(
		"res://addons/gdUnit4/test/core/resources/scenes/input_actions/InputEventTestScene.tscn"
	)
...
func test_simulate_mouse_button_press_left_and_right() -> void:
	...
	assert_bool(Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT)).is_true()
	assert_bool(Input.is_mouse_button_pressed(MOUSE_BUTTON_RIGHT)).is_true()
	assert_that(Input.get_mouse_button_mask()).is_equal(

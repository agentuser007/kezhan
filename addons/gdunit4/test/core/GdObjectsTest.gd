...
func test_equals_Node_with_deep_check() -> void:
	...
	(
		assert_bool(GdObjects.equals(nodeA, nodeA, false, GdObjects.COMPARE_MODE.OBJECT_REFERENCE))
		. is_true()
	)
	(
		assert_bool(GdObjects.equals(nodeB, nodeB, false, GdObjects.COMPARE_MODE.OBJECT_REFERENCE))
		. is_true()
	)
	(
		assert_bool(GdObjects.equals(nodeA, nodeB, false, GdObjects.COMPARE_MODE.OBJECT_REFERENCE))
		. is_false()
	)
	(
		assert_bool(GdObjects.equals(nodeB, nodeA, false, GdObjects.COMPARE_MODE.OBJECT_REFERENCE))
		. is_false()
	)
	...
	for singleton_name in Engine.get_singleton_list():
		...
		(
			...
			. override_failure_message(
				(
					"Expect to a singleton: '%s' Instance: %s, Class: %s"
					% [singleton_name, singleton, singleton.get_class()]
				)
			)
			. is_true()
		)
...
func test_get_class_name_from_class_path() -> void:
	...
	(
		...
		. is_success()
		. is_value("CustomClass")
	)
	(
		assert_result(
			extract_class_name("res://addons/gdUnit4/test/mocker/resources/CustomNodeTestClass.gd")
		)
		. is_success()
		. is_value("CustomNodeTestClass")
	)
	(
		assert_result(
			extract_class_name(
				"res://addons/gdUnit4/test/mocker/resources/CustomResourceTestClass.gd"
			)
		)
		. is_success()
		. is_value("CustomResourceTestClass")
	)
	(
		assert_result(
			extract_class_name(
				"res://addons/gdUnit4/test/mocker/resources/OverridenGetClassTestClass.gd"
			)
		)
		. is_success()
		. is_value("OverridenGetClassTestClass")
	)
...
func test_get_class_name_from_snake_case_class_path() -> void:
	(
		assert_result(
			extract_class_name(
				"res://addons/gdUnit4/test/core/resources/naming_conventions/snake_case_with_class_name.gd"
			)
		)
		. is_success()
		. is_value("SnakeCaseWithClassName")
	)
	# without class_name
	(
		assert_result(
			extract_class_name(
				"res://addons/gdUnit4/test/core/resources/naming_conventions/snake_case_without_class_name.gd"
			)
		)
		. is_success()
		. is_value("SnakeCaseWithoutClassName")
	)
...
func test_get_class_name_from_pascal_case_class_path() -> void:
	(
		assert_result(
			extract_class_name(
				"res://addons/gdUnit4/test/core/resources/naming_conventions/PascalCaseWithClassName.gd"
			)
		)
		. is_success()
		. is_value("PascalCaseWithClassName")
	)
	# without class_name
	(
		assert_result(
			extract_class_name(
				"res://addons/gdUnit4/test/core/resources/naming_conventions/PascalCaseWithoutClassName.gd"
			)
		)
		. is_success()
		. is_value("PascalCaseWithoutClassName")
	)
...
func test_get_class_name_from_type() -> void:
	...
	assert_result(extract_class_name(CustomNodeTestClass)).is_success().is_value(
		"CustomNodeTestClass"
	)
	assert_result(extract_class_name(CustomResourceTestClass)).is_success().is_value(
		"CustomResourceTestClass"
	)
	assert_result(extract_class_name(OverridenGetClassTestClass)).is_success().is_value(
...
func test_get_class_name_from_inner_class() -> void:
	assert_result(extract_class_name(CustomClass)).is_success().is_value("CustomClass")
	assert_result(extract_class_name(CustomClass.InnerClassA)).is_success().is_value(
		"CustomClass.InnerClassA"
	)
	assert_result(extract_class_name(CustomClass.InnerClassB)).is_success().is_value(
		"CustomClass.InnerClassB"
	)
	assert_result(extract_class_name(CustomClass.InnerClassC)).is_success().is_value(
		"CustomClass.InnerClassC"
	)
	assert_result(extract_class_name(CustomClass.InnerClassD)).is_success().is_value(
		"CustomClass.InnerClassD"
	)
	assert_result(extract_class_name(AdvancedTestClass.SoundData)).is_success().is_value(
		"AdvancedTestClass.SoundData"
	)
	assert_result(extract_class_name(AdvancedTestClass.AtmosphereData)).is_success().is_value(
		"AdvancedTestClass.AtmosphereData"
	)
	assert_result(extract_class_name(AdvancedTestClass.Area4D)).is_success().is_value(
		"AdvancedTestClass.Area4D"
	)
...
func test_extract_class_name_from_instance() -> void:
	...
	var classWithoutNameA: GDScript = load(
		"res://addons/gdUnit4/test/mocker/resources/ClassWithoutNameA.gd"
	)
	...
	assert_result(extract_class_name(CustomResourceTestClass.new())).is_equal(
		"CustomResourceTestClass"
	)
	assert_result(extract_class_name(OverridenGetClassTestClass.new())).is_equal(
		"OverridenGetClassTestClass"
	)
	assert_result(extract_class_name(AdvancedTestClass.new())).is_equal("AdvancedTestClass")
	# extract inner class name
	assert_result(extract_class_name(AdvancedTestClass.SoundData.new())).is_equal(
		"AdvancedTestClass.SoundData"
	)
	assert_result(extract_class_name(AdvancedTestClass.AtmosphereData.new())).is_equal(
		"AdvancedTestClass.AtmosphereData"
	)
	assert_result(extract_class_name(AdvancedTestClass.Area4D.new(0))).is_equal(
		"AdvancedTestClass.Area4D"
	)
	assert_result(extract_class_name(CustomClass.InnerClassC.new())).is_equal(
		"CustomClass.InnerClassC"
	)
...
func test_extract_class_name_from_class_path(
	fuzzer := GodotClassNameFuzzer.new(true, true), fuzzer_iterations := 100
) -> void:
	var clazz_name: String = fuzzer.next_value()
	(
		assert_str(GdObjects.extract_class_name_from_class_path(PackedStringArray([clazz_name])))
		. is_equal(clazz_name)
	)
...
func test_extract_class_path_by_clazz() -> void:
	...
	# script classes
	assert_array(GdObjects.extract_class_path(Person)).contains_exactly(
		["res://addons/gdUnit4/test/resources/core/Person.gd"]
	)
	assert_array(GdObjects.extract_class_path(CustomClass)).contains_exactly(
		["res://addons/gdUnit4/test/resources/core/CustomClass.gd"]
	)
	assert_array(GdObjects.extract_class_path(CustomNodeTestClass)).contains_exactly(
		["res://addons/gdUnit4/test/mocker/resources/CustomNodeTestClass.gd"]
	)
	assert_array(GdObjects.extract_class_path(CustomResourceTestClass)).contains_exactly(
		["res://addons/gdUnit4/test/mocker/resources/CustomResourceTestClass.gd"]
	)
	assert_array(GdObjects.extract_class_path(OverridenGetClassTestClass)).contains_exactly(
		["res://addons/gdUnit4/test/mocker/resources/OverridenGetClassTestClass.gd"]
	)

	# script inner classes
	assert_array(GdObjects.extract_class_path(CustomClass.InnerClassA)).contains_exactly(
		["res://addons/gdUnit4/test/resources/core/CustomClass.gd", "InnerClassA"]
	)
	assert_array(GdObjects.extract_class_path(CustomClass.InnerClassB)).contains_exactly(
		["res://addons/gdUnit4/test/resources/core/CustomClass.gd", "InnerClassB"]
	)
	assert_array(GdObjects.extract_class_path(CustomClass.InnerClassC)).contains_exactly(
		["res://addons/gdUnit4/test/resources/core/CustomClass.gd", "InnerClassC"]
	)
	assert_array(GdObjects.extract_class_path(AdvancedTestClass.SoundData)).contains_exactly(
		["res://addons/gdUnit4/test/mocker/resources/AdvancedTestClass.gd", "SoundData"]
	)
	assert_array(GdObjects.extract_class_path(AdvancedTestClass.AtmosphereData)).contains_exactly(
		["res://addons/gdUnit4/test/mocker/resources/AdvancedTestClass.gd", "AtmosphereData"]
	)
	assert_array(GdObjects.extract_class_path(AdvancedTestClass.Area4D)).contains_exactly(
		["res://addons/gdUnit4/test/mocker/resources/AdvancedTestClass.gd", "Area4D"]
	)
...
func test_create_instance_by_class_name() -> void:
	...
	assert_object(create_instance(Node)).is_not_null().is_instanceof(Node)
	assert_object(create_instance(Camera3D)).is_not_null().is_instanceof(Camera3D)
	# instance of custom classes
	assert_object(create_instance(Person)).is_not_null().is_instanceof(Person)
	# instance of inner classes
	assert_object(create_instance(CustomClass.InnerClassA)).is_not_null().is_instanceof(
		CustomClass.InnerClassA
	)
...
func test_extract_class_name_on_null_value() -> void:
	...
	assert_result(GdObjects.extract_class_name(null)).is_error().contains_message(
		"Can't extract class name form a null value."
	)
...
func test_is_instance_scene() -> void:
	...
	var resource: PackedScene = load(
		"res://addons/gdUnit4/test/mocker/resources/scenes/TestScene.tscn"
	)
...
func test_is_scene_resource_path() -> void:
	...
	(
		assert_bool(
			GdObjects.is_scene_resource_path(
				"res://addons/gdUnit4/test/mocker/resources/scenes/TestScene.tscn"
			)
		)
		. is_true()
...
func test_extract_class_functions() -> void:
	...
	for f: Dictionary in functions:
		if f["name"] == "get_path":
			assert_str(GdFunctionDescriptor.extract_from(f)._to_string()).is_equal(
				"[Line:-1] func get_path() -> String:"
			)

	functions = GdObjects.extract_class_functions(
		"CustomResourceTestClass",
		["res://addons/gdUnit4/test/mocker/resources/CustomResourceTestClass.gd"]
	)
	for f: Dictionary in functions:
...
func test_is_equal_on_scene_embedded_script() -> void:
	...
	var node: Node = auto_free(
		(
			load("res://addons/gdUnit4/test/core/resources/scenes/SceneWithEmbeddedScript.tscn")
			. instantiate()
		)
	)

...
func test_to_string_failure_with_line_number() -> void:
	...
	assert_str(report.to_string()).is_equal(
		"[color=green]line [/color][color=aqua]42:[/color] test message"
	)
...
func test_to_string_failure_with_unknown_line() -> void:
	...
	assert_str(report.to_string()).is_equal(
		"[color=green]line [/color][color=aqua]<n/a>:[/color] test message"
	)
...
func test_stack_trace_returns_trace_from_error() -> void:
	...
	var report := GdUnitReport.new().from_error(
		GdUnitReport.FAILURE, GdUnitError.new("test message", 10, trace)
	)
...
func test_serialize_report_contains_all_base_fields() -> void:
	var serialized := (
		GdUnitReport.new().create(GdUnitReport.FAILURE, 42, "test message").serialize()
	)
	(
...
func test_serialize_report_with_error_includes_stack_trace_key() -> void:
	...
	var serialized := (
		GdUnitReport
		. new()
		. from_error(GdUnitReport.FAILURE, GdUnitError.new("test message", 10, trace))
		. serialize()
	)
...
func test_deserialize_without_stack_trace_produces_full_report() -> void:
	...
	assert_that(GdUnitReport.new().deserialize(serialized)).is_equal(
		GdUnitReport.new().create(GdUnitReport.FAILURE, 42, "test message")
	)
...
func test_deserialize_reconstructs_full_report() -> void:
	...
	var expected := GdUnitReport.new().from_error(
		GdUnitReport.FAILURE, GdUnitError.new("test message", 10, trace)
	)
	var serialized := {

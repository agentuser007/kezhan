...
func test_is_greater_major(
	fuzzer_major := Fuzzers.rangei(1, 20),
	fuzzer_minor := Fuzzers.rangei(0, 20),
	fuzzer_patch := Fuzzers.rangei(0, 20),
	fuzzer_iterations := 500
) -> void:
	var version := GdUnit4Version.new(0, 9, 1)
	@warning_ignore("unsafe_cast")
	var current := GdUnit4Version.new(
		fuzzer_major.next_value() as int,
		fuzzer_minor.next_value() as int,
		fuzzer_patch.next_value() as int
	)
	(
...
func test_is_not_greater_major(
	fuzzer_major := Fuzzers.rangei(1, 10),
	fuzzer_minor := Fuzzers.rangei(0, 20),
	fuzzer_patch := Fuzzers.rangei(0, 20),
	fuzzer_iterations := 500
) -> void:
...
func test_is_greater_minor(
	fuzzer_minor := Fuzzers.rangei(3, 20),
	fuzzer_patch := Fuzzers.rangei(0, 20),
	fuzzer_iterations := 500
) -> void:
	...
	@warning_ignore("unsafe_cast")
	var current := GdUnit4Version.new(
		0, fuzzer_minor.next_value() as int, fuzzer_patch.next_value() as int
	)
	(
		assert_bool(current.is_greater(version))
		. override_failure_message("Expect %s is greater then %s" % [current, version])

...
func test_as_string(
	...
	_test_parameters := [
		...
		[
			"Array",
			Array([Vector2.ZERO, Vector2.LEFT]),
			"[Vector2(), Vector2" + str(Vector2(-1, 0)) + "]"
		],
		[
			"Array",
			Array([Vector3.ZERO, Vector3.LEFT]),
			"[Vector3(), Vector3" + str(Vector3(-1, 0, 0)) + "]"
		],
		[
			"Array",
			Array([Color.RED, Color.GREEN]),
			"[Color" + str(Color(1, 0, 0, 1)) + ", Color" + str(Color(0, 1, 0, 1)) + "]"
		],
		...
		[
			"ArrayVector2",
			Array([Vector2.ZERO, Vector2.LEFT]) as Array[Vector2],
			"[Vector2(), Vector2" + str(Vector2(-1, 0)) + "]"
		],
		[
			"ArrayVector2i",
			Array([Vector2i.ZERO, Vector2i.LEFT]) as Array[Vector2i],
			"[Vector2i(), Vector2i" + str(Vector2i(-1, 0)) + "]"
		],
		[
			"ArrayVector3",
			Array([Vector3.ZERO, Vector3.LEFT]) as Array[Vector3],
			"[Vector3(), Vector3" + str(Vector3(-1, 0, 0)) + "]"
		],
		[
			"ArrayVector3i",
			Array([Vector3i.ZERO, Vector3i.LEFT]) as Array[Vector3i],
			"[Vector3i(), Vector3i" + str(Vector3i(-1, 0, 0)) + "]"
		],
		[
			"ArrayVector4",
			Array([Vector4.ZERO, Vector4.ONE]) as Array[Vector4],
			"[Vector4(), Vector4%s]" % Vector4(1, 1, 1, 1)
		],
		[
			"ArrayVector4i",
			Array([Vector4i.ZERO, Vector4i.ONE]) as Array[Vector4i],
			"[Vector4i(), Vector4i(1, 1, 1, 1)]"
		],
		[
			"ArrayColor",
			Array([Color.RED, Color.GREEN]) as Array[Color],
			"[Color" + str(Color(1, 0, 0, 1)) + ", Color" + str(Color(0, 1, 0, 1)) + "]"
		],
		...
		[
			"PackedFloat32Array",
			PackedFloat32Array([1, 2.212]),
			"PackedFloat32Array[1.000000, 2.212000]"
		],
		[
			"PackedFloat64Array",
			PackedFloat64Array([1, 2.212]),
			"PackedFloat64Array[1.000000, 2.212000]"
		],
		["PackedStringArray", PackedStringArray([1, 2]), 'PackedStringArray["1", "2"]'],
		[
			"PackedVector2Array",
			PackedVector2Array([Vector2.ZERO, Vector2.LEFT]),
			"PackedVector2Array[Vector2(), Vector2" + str(Vector2(-1, 0)) + "]"
		],
		[
			"PackedVector3Array",
			PackedVector3Array([Vector3.ZERO, Vector3.LEFT]),
			"PackedVector3Array[Vector3(), Vector3" + str(Vector3(-1, 0, 0)) + "]"
		],
		[
			"PackedColorArray",
...
func test_filter_value(
	...
	_test_parameters := [
		...
		[
			PackedVector2Array([Vector2.ZERO, Vector2.ONE, Vector2.DOWN, Vector2.ZERO]),
			TYPE_PACKED_VECTOR2_ARRAY
		],
		[
			PackedVector3Array([Vector3.ZERO, Vector3.ONE, Vector3.DOWN, Vector3.ZERO]),
...
func test_group_by() -> void:
	...
	# We group by type
	var result := GdArrayTools.group_by(values, func(item: ExampleItem) -> int: return item._type)

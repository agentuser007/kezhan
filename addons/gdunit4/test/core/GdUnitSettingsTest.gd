		...
		. contains_exactly_in_any_order(
			[
				...
				tuple(
					...
					3,
					3,
					"Maximum retry count on test failure",
					PackedStringArray()
				),
			]
		)
...
func test_property_bool_info() -> void:
	...
	GdUnitSettings.create_property_if_need(
		property_name, true, "Show notification when a new version is found"
	)
...
func test_property_int_info() -> void:
	...
	GdUnitSettings.create_property_if_need(
		property_name, 30, "Server connection timeout in minutes"
	)
...
func test_property_string_info() -> void:
	...
	GdUnitSettings.create_property_if_need(
		property_name, "test", "Subfolder where test suites are located"
	)
...
func test_property_enum_info() -> void:
	...
	GdUnitSettings.create_property_if_need(
		...
		"Naming convention for test suite generation",
		value_set
	)
	...
	assert_that(ProjectSettings.property_get_revert(property_name)).is_equal(
		GdUnitSettings.NAMING_CONVENTIONS.AUTO_DETECT
	)
...
func test_migrate_property_change_key() -> void:
	...
	GdUnitSettings.migrate_property(
		old_property.name(), new_property_X, old_property.default(), old_property.help()
	)
...
func test_validate_is_inferred_declaration_enabled_when_disabled() -> void:
	...
	assert_result(GdUnitSettings.validate_is_inferred_declaration_enabled()).is_success()
...
func test_validate_is_inferred_declaration_enabled_when_warning_and_addon_excluded() -> void:
	...
	if Engine.get_version_info().hex >= 0x40600:
		ProjectSettings.set_setting(
			GdUnitSettings.GDSCRIPT_WARNINGS_DIRECTORY_RULES, {"res://addons/gdUnit4": EXCLUDE}
		)
	else:
...
func test_validate_is_inferred_declaration_enabled_when_warning_and_parent_dir_excluded() -> void:
	...
	if Engine.get_version_info().hex >= 0x40600:
		...
		ProjectSettings.set_setting(
			GdUnitSettings.GDSCRIPT_WARNINGS_DIRECTORY_RULES, {"res://addons": EXCLUDE}
		)
	else:
		ProjectSettings.set_setting(GdUnitSettings.GDSCRIPT_WARNINGS_EXCLUDE_ADDONS, true)
	assert_result(GdUnitSettings.validate_is_inferred_declaration_enabled()).is_success()
...
func test_validate_is_inferred_declaration_enabled_when_warning_and_addon_excluded_but_parent_included(
) -> void:
	ProjectSettings.set_setting(GdUnitSettings.GDSCRIPT_WARNINGS_INFERRED_DECLARATION, WARN)
	if Engine.get_version_info().hex >= 0x40600:
		# parent addons dir is included (warnings active) but gdUnit4 is specifically excluded
		ProjectSettings.set_setting(
			GdUnitSettings.GDSCRIPT_WARNINGS_DIRECTORY_RULES,
			{"res://addons": INCLUDE, "res://addons/gdUnit4": EXCLUDE}
		)
	else:
		ProjectSettings.set_setting(GdUnitSettings.GDSCRIPT_WARNINGS_EXCLUDE_ADDONS, true)
	assert_result(GdUnitSettings.validate_is_inferred_declaration_enabled()).is_success()
...
func test_validate_is_inferred_declaration_enabled_when_warning_and_only_parent_included() -> void:
	...
	if Engine.get_version_info().hex >= 0x40600:
		...
		ProjectSettings.set_setting(
			GdUnitSettings.GDSCRIPT_WARNINGS_DIRECTORY_RULES, {"res://addons": INCLUDE}
		)
		expected_message = (
			"""
...
func test_validate_is_inferred_declaration_enabled_when_warning_and_addon_not_excluded() -> void:
	...
	if Engine.get_version_info().hex >= 0x40600:
		...
		ProjectSettings.set_setting(
			GdUnitSettings.GDSCRIPT_WARNINGS_DIRECTORY_RULES,
			{"res://addons": EXCLUDE, "res://addons/gdUnit4": INCLUDE}
		)
		expected_message = (
			"""
			GdUnit4: 'inferred_declaration' is set to Warning/Error!
...
func test_migrate_properties_v215() -> void:
	...
	GdUnitSettings.create_property_if_need(
		old_property, "test", "Sets the root folder where test-suites located/generated."
	)

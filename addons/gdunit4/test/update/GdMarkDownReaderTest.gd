...
func test_tobbcode() -> void:
	var source := resource_as_string(
		"res://addons/gdUnit4/test/update/resources/markdown_example.txt"
	)
	var expected := resource_as_string(
		"res://addons/gdUnit4/test/update/resources/bbcode_example.txt"
	)
...
func test_tobbcode_table() -> void:
	var source := resource_as_string(
		"res://addons/gdUnit4/test/update/resources/markdown_table.txt"
	)
	var expected := resource_as_string(
		"res://addons/gdUnit4/test/update/resources/bbcode_table.txt"
	)
	assert_str(await _reader.to_bbcode(source)).is_equal(expected)
...
func test_tobbcode_html_headers() -> void:
	...
	var expected := resource_as_string(
		"res://addons/gdUnit4/test/update/resources/bbcode_header.txt"
	)
	assert_str(await _reader.to_bbcode(source)).is_equal(expected)
...
func test_tobbcode_md_headers() -> void:
	...
	var expected := resource_as_string(
		"res://addons/gdUnit4/test/update/resources/bbcode_md_header.txt"
	)
	assert_str(await _reader.to_bbcode(source)).is_equal(expected)


func test_tobbcode_list() -> void:
	assert_str(await _reader.to_bbcode("- item")).is_equal(
		"[img=12x12]res://addons/gdUnit4/src/update/assets/dot1.png[/img] item"
	)
	assert_str(await _reader.to_bbcode("  - item")).is_equal(
		"   [img=12x12]res://addons/gdUnit4/src/update/assets/dot2.png[/img] item"
	)
	assert_str(await _reader.to_bbcode("    - item")).is_equal(
		"      [img=12x12]res://addons/gdUnit4/src/update/assets/dot1.png[/img] item"
	)
...
func test_to_bbcode_embeded_text() -> void:
	assert_str(await _reader.to_bbcode("> some text")).is_equal(
		"[img=50x14]res://addons/gdUnit4/src/update/assets/embedded.png[/img][i] some text[/i]"
	)
...
func test_process_image() -> void:
	...
	(
		assert_str(
			await _reader.process_image(
				reg_ex, "![alt text](res://addons/gdUnit4/test/update/resources/icon48.png)"
			)
		)
	...
	assert_str(await _reader.process_image(reg_ex, input)).is_equal(expected)
...
func test_process_image_by_reference() -> void:
	...
	# without tooltip
	assert_str(_reader.process_image_references(reg_ex, input)).is_equal(expected)

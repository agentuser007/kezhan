...
func test_add_child() -> void:
	var child := XmlElement.new("foo").attribute(JUnitXmlReportWriter.ATTR_ID, "1").attribute(
		JUnitXmlReportWriter.ATTR_NAME, "foo"
	)
	var element := (
		XmlElement
		. new("bar")
...
func test_add_childs() -> void:
	var child_a := XmlElement.new("foo_a").attribute(JUnitXmlReportWriter.ATTR_ID, 1).attribute(
		JUnitXmlReportWriter.ATTR_NAME, "foo_a"
	)
	var child_b := XmlElement.new("foo_b").attribute(JUnitXmlReportWriter.ATTR_ID, 2).attribute(
		JUnitXmlReportWriter.ATTR_NAME, "foo_b"
	)
	var element := (
		XmlElement
		. new("bar")
...
func test_add_text() -> void:
	var element := XmlElement.new("testsuites").text("This is a message")
	var expected := (
		"""
		<testsuites>
		<![CDATA[

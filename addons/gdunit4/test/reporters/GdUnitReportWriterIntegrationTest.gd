...
func _on_test_event(event: GdUnitEvent) -> void:
	match event.type():
		GdUnitEvent.TESTSUITE_BEFORE:
			...
			_report_summary.add_testsuite_report(
		...
		GdUnitEvent.TESTSUITE_AFTER:
			...
			_report_summary.add_testsuite_reports(event.resource_path(), event.reports())
...
func _load_test_cases(suite_resource_path: String) -> Array[GdUnitTestCase]:
	var suite_failing_tests := GdUnitTestResourceLoader.load_tests(
		RESOURCE_SUITES + suite_resource_path
	)
...
func _assert_report_matches(actual_path: String, expected_path: String) -> void:
	...
	var expected := _patch_out_timings(
		FileAccess.open(expected_path, FileAccess.READ).get_as_text()
	)

...
func test_breadcrumb_href_in_suite_report_replaces_spaces_with_underscores() -> void:
	var suite_html := (
		FileAccess
		. open(_report_dir + "/test_suites/my_test_suite.SomeTest.html", FileAccess.READ)
		. get_as_text()
	)
...
func test_suite_file_uses_underscores_not_spaces() -> void:
	(
		assert_bool(
			FileAccess.file_exists(_report_dir + "/test_suites/my_test_suite.SomeTest.html")
		)
		. is_true()
	)
	(
		assert_bool(
			FileAccess.file_exists(_report_dir + "/test_suites/my test suite.SomeTest.html")
		)
		. is_false()
	)
#endregion

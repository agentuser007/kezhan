...
func _make_suite_report(resource_path: String, suite_name: String) -> GdUnitTestSuiteReport:
	return GdUnitTestSuiteReport.new(
		resource_path, suite_name, 1, func(s: String) -> String: return s
	)
...
func test_write_html_file_creates_file_with_content() -> void:
	...
	assert_str(FileAccess.open(output_path, FileAccess.READ).get_as_text()).is_equal(
		"<html>test</html>"
	)

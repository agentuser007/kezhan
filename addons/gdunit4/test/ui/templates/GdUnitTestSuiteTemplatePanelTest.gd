...
func test_show() -> void:
	var template: Variant = spy(
		"res://addons/gdUnit4/src/ui/templates/GdUnitTestSuiteTemplatePanel.tscn"
	)
...
func test_load_template_gd() -> void:
	...
	assert_int(runner.get_property("_selected_template")).is_equal(
		GdUnitTestSuiteTemplate.TEMPLATE_ID_GD
	)
	assert_str(runner.get_property("_template_editor").text).is_equal(
		GdUnitTestSuiteTemplate.default_GD_template().replace("\r", "")
	)
...
func test_load_template_cs() -> void:
	...
	assert_int(runner.get_property("_selected_template")).is_equal(
		GdUnitTestSuiteTemplate.TEMPLATE_ID_CS
	)
	assert_str(runner.get_property("_template_editor").text).is_equal(
		GdUnitTestSuiteTemplate.default_CS_template().replace("\r", "")
	)

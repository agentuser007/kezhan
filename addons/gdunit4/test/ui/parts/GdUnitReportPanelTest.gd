...
func _build_report_with_frames(
	message: String, frames: Array[GdUnitStackTraceElement]
) -> GdUnitReport:
	return GdUnitReport.new().from_error(
		GdUnitReport.FAILURE, GdUnitError.new(message, 1, GdUnitStackTrace.of(frames))
	)
...
func test_meta_clicked_signal_is_connected_when_stack_trace_has_frames() -> void:
	...
	var label: RichTextLabel = auto_free(
		_panel.build_report(_build_report_with_frames("test", frames))
	)
...
func test_on_meta_clicked_is_called_with_expected_frame_on_click() -> void:
	...
	var frame1 := GdUnitStackTraceElement.new("res://test/MyTest.gd", 42, "test_foo")
	# Spy on the panel via the scene so that _on_meta_clicked calls are recorded
	var panel_spy: GdUnitReportPanel = spy(
		"res://addons/gdUnit4/src/ui/parts/GdUnitReportPanel.tscn"
	)
	...
	var click_pos := Vector2(
		label_rect.position.x + 50, label_rect.position.y + label.get_line_offset(1) + 5
	)
...
func test_build_report_parsed_text_contains_message() -> void:
	var label: RichTextLabel = auto_free(
		_panel.build_report(_build_report("expected failure text"))
	)
...
func test_build_report_with_stack_depth_of_1() -> void:
	...
	var label: RichTextLabel = auto_free(
		_panel.build_report(_build_report_with_frames("assertion failed", frames))
	)
	(
		assert_str(label.get_parsed_text())
		. is_equal(
			(
				"""
		assertion failed

...
const GdUnitInspectorTreeMainPanel := preload(
	"res://addons/gdUnit4/src/ui/parts/GdUnitInspectorTreeMainPanel.gd"
)
...
func before_test() -> void:
	...
	_inspector = (
		load("res://addons/gdUnit4/src/ui/parts/GdUnitInspectorTreeMainPanel.tscn").instantiate()
	)
...
func setup_test_env() -> void:
	var suite_a := GdUnitTestResourceLoader.load_gd_script(
		"res://addons/gdUnit4/test/ui/parts/resources/foo/ExampleTestSuiteA.resource", true
	)
	var suite_b := GdUnitTestResourceLoader.load_gd_script(
		"res://addons/gdUnit4/test/ui/parts/resources/foo/ExampleTestSuiteB.resource", true
	)
	var suite_c := GdUnitTestResourceLoader.load_gd_script(
		"res://addons/gdUnit4/test/ui/parts/resources/foo/ExampleTestSuiteC.resource", true
	)

	GdUnitTestDiscoverer.discover_tests(
		suite_a,
		func(discover_test: GdUnitTestCase) -> void:
	...
	suite_a_item = _inspector._find_tree_item_by_test_suite(
		_inspector._tree_root, suite_a.resource_path, "ExampleTestSuiteA"
	)
	suite_b_item = _inspector._find_tree_item_by_test_suite(
		_inspector._tree_root, suite_b.resource_path, "ExampleTestSuiteB"
	)
	suite_c_item = _inspector._find_tree_item_by_test_suite(
		_inspector._tree_root, suite_c.resource_path, "ExampleTestSuiteC"
	)


func set_test_state(
	test_cases: Array[GdUnitTestCase], state: GdUnitInspectorTreeMainPanel.STATE
) -> void:
...
func test_find_item_by_id() -> void:
	var suite_script := GdUnitTestResourceLoader.load_gd_script(
		"res://addons/gdUnit4/test/ui/parts/resources/bar/ExampleTestSuiteA.resource", true
	)
...
func test_find_item_by_id_GD809() -> void:
	var suite_script := GdUnitTestResourceLoader.load_gd_script(
		"res://addons/gdUnit4/test/ui/parts/resources/gd_809/test_example.resource"
	)
	var discovered_tests := {}
	GdUnitTestDiscoverer.discover_tests(
		suite_script,
...
func test_find_item_by_path_GD809() -> void:
	...
	var item := _inspector._find_tree_item_by_test_suite(
		_inspector._tree_root, test_example.suite_resource_path, "test_example"
	)
	assert_object(item).is_not_null()
	# find tests by id
	(
		assert_object(_inspector._find_tree_item_by_id(_inspector._tree_root, test_example.guid))
		. is_not_null()
	)
	(
		assert_object(_inspector._find_tree_item_by_id(_inspector._tree_root, test_example_b.guid))
		. is_not_null()
	)
...
func test_suite_text_responds_to_test_case_events() -> void:
	...
	var error_ad := GdUnitEvent.new().test_after(
		test_ad.guid, "test_ad", {GdUnitEvent.ERRORS: true}
	)
	_inspector._on_gdunit_event(error_ad)
	assert_str(suite_a_item.get_text(0)).is_equal("(1/5) ExampleTestSuiteA")

	var failure_ab := GdUnitEvent.new().test_after(
		test_ab.guid, "test_ab", {GdUnitEvent.FAILED: true}
	)
	_inspector._on_gdunit_event(failure_ab)
	assert_str(suite_a_item.get_text(0)).is_equal("(1/5) ExampleTestSuiteA")

	var skipped_ac := GdUnitEvent.new().test_after(
		test_ac.guid, "test_ac", {GdUnitEvent.SKIPPED: true}
	)
...
func test_update_test_case_on_multiple_test_suite_with_same_name() -> void:
	...
	var suite_item := _inspector._find_tree_item_by_test_suite(
...
func test_update_icon_state() -> void:
	...
	var suite_item := _inspector._find_tree_item_by_test_suite(
		_inspector._tree_root, suite_script_path, suite_name
	)
	...
	_inspector._on_gdunit_event(
		GdUnitEvent.new().test_after(test_case2.guid, "test_case2", {GdUnitEvent.FAILED: true})
	)
	# We check whether a test event with a warning does not overwrite a higher object status, e.g. an error.
	_inspector._on_gdunit_event(
		GdUnitEvent.new().test_after(test_case2.guid, "test_case2", {GdUnitEvent.WARNINGS: true})
	)
...
func test_custom_sort_by_original_index() -> void:
	...
	for n in 10:
		...
		GdUnitInspectorTreeMainPanel._sort_tree_items(
			tree_root, GdUnitInspectorTreeConstants.SORT_MODE.UNSORTED
		)
		# verify
		(
			assert_array(tree_root.get_children())
			. extractv(ItemNameExtractor.new())
			. contains_exactly(
				[
...
func test_custom_sort_by_name_ascending() -> void:
	...
	for n in 10:
		...
		GdUnitInspectorTreeMainPanel._sort_tree_items(
			tree_root, GdUnitInspectorTreeConstants.SORT_MODE.NAME_ASCENDING
		)
		# verify
		(
			assert_array(tree_root.get_children())
...
func test_custom_sort_by_name_descending() -> void:
	...
	for n in 10:
		...
		GdUnitInspectorTreeMainPanel._sort_tree_items(
			tree_root, GdUnitInspectorTreeConstants.SORT_MODE.NAME_DESCENDING
		)
		# verify
		(
			assert_array(tree_root.get_children())
...
func test_custom_sort_by_execution_time() -> void:
	...
	for n in 10:
		...
		GdUnitInspectorTreeMainPanel._sort_tree_items(
			tree_root, GdUnitInspectorTreeConstants.SORT_MODE.EXECUTION_TIME
		)
		# verify
		(
			assert_array(tree_root.get_children())
...
func create_test_item(parent: TreeItem, test_name: String, execution_time := 0) -> TreeItem:
	...
	item.set_meta(
		GdUnitInspectorTreeMainPanel.META_GDUNIT_TYPE,
		GdUnitInspectorTreeMainPanel.GdUnitType.TEST_CASE
	)
...
func create_folder_item(parent: TreeItem, folder_name: String, execution_time := 0) -> TreeItem:
	...
	item.set_meta(
		GdUnitInspectorTreeMainPanel.META_GDUNIT_TYPE,
		GdUnitInspectorTreeMainPanel.GdUnitType.FOLDER
	)
...
func test_discover_tests() -> void:
	...
	(
		assert_bool(
			GdUnitSignals.instance().gdunit_test_discover_added.is_connected(
				_inspector.on_test_case_discover_added
			)
		)
		. override_failure_message(
			"The 'InspectorProgressBar' must be connected to signal 'gdunit_test_discovered'"
		)
		. is_true()
	)
...
func test_on_test_case_discover_added() -> void:
	...
	_inspector.on_test_case_discover_added(
		GdUnitTestCase.from(
			"res://addons/gdUnit4/test/dir_a/dir_b/my_test_suite.gd",
			"res://addons/gdUnit4/test/dir_a/dir_b/my_test_suite.gd",
			0,
			"test_foo"
		)
	...
	_inspector.on_test_case_discover_added(
		GdUnitTestCase.from(
			"res://addons/gdUnit4/test/dir_a/dir_x/my_test_suite2.gd",
			"res://addons/gdUnit4/test/dir_a/dir_x/my_test_suite2.gd",
			0,
			"test_foo"
		)
	...
	_inspector.on_test_case_discover_added(
		GdUnitTestCase.from("res://my_test_suite3.gd", "res://my_test_suite3.gd", 0, "test_foo")
	)
...
func test_collect_test_cases() -> void:
	var script := load_non_cached(
		"res://addons/gdUnit4/test/core/discovery/resources/DiscoverExampleTestSuite.gd"
	)
	var tests_by_id := {}
	GdUnitTestDiscoverer.discover_tests(
		script,
		func(test_to_discover: GdUnitTestCase) -> void:
	...
	# Do verify all tests are collected, ignoring the order could be different according to selected sort mode
	var expected_tests: Array = tests_by_id.values().filter(
		func(test_to_filter: GdUnitTestCase) -> bool:
			return test_to_filter.test_name == "test_parameterized_static"
	)
	assert_array(collected_tests).has_size(3).contains_exactly_in_any_order(expected_tests)


func test_collect_test_cases_GD_872(
	_do_skip := not GdUnit4CSharpApiLoader.is_api_loaded(),
	_skip_reason := "Do run only for Godot .Net version"
) -> void:
	var tests_by_id := {}
	...
	# Validate tree paths
	assert_str(buld_test_case_tree_path(test_case1)).is_equal(
		"tree_root/Minimal/Tests/ATests/TestExample"
	)
	assert_str(buld_test_case_tree_path(test_case2)).is_equal(
		"tree_root/Minimal/Tests/ATests/TestExample2"
	)
	# Valide suite find by resource name
	(
		assert_object(
			_inspector._find_tree_item_by_test_suite(
				_inspector._tree_root, test_case1.suite_resource_path, "ATests"
			)
		)
		. is_not_null()
	)
	...
	# Validate tree paths
	assert_str(buld_test_case_tree_path(test_case1)).is_equal(
		"tree_root/Minimal/Tests/AZTests/TestExample"
	)
	assert_str(buld_test_case_tree_path(test_case2)).is_equal(
		"tree_root/Minimal/Tests/AZTests/TestExample2"
	)
	# Valide suite find by resource name
	(
		assert_object(
			_inspector._find_tree_item_by_test_suite(
				_inspector._tree_root, test_case1.suite_resource_path, "AZTests"
			)
		)
		. is_not_null()
	)
...
func assert_tree_equals(tree_left: TreeItem, tree_right: TreeItem) -> bool:
	...
	(
		...
		. override_failure_message(
			(
				"Expecting same child count %d vs %d on item %s"
				% [left_childs.size(), right_childs.size(), tree_left.get_meta("gdUnit_name")]
			)
		)
	...
	for index in left_childs.size():
		...
		(
			...
			. override_failure_message(
				"Expecting '%s' == '%s'" % [get_item_name(l), get_item_name(r)]
			)

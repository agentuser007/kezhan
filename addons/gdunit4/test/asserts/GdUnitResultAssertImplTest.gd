...
func test_is_null() -> void:
	...
	(
		...
		. is_failed()
		. has_message(
			'Expecting: \'<null>\' but was <{ "state": 0, "value": "\\"\\"", "warn_msg": "", "err_msg": "" }>'
		)
	)
...
func test_contains_message() -> void:
	...
	(
		assert_failure(
			func() -> void: assert_result(GdUnitResult.success("")).contains_message("Error 500")
		)
		. is_failed()
	...
	(
		assert_failure(
			func() -> void:
				assert_result(GdUnitResult.warn("Warning xyz!")).contains_message("Warning aaa!")
		)
		. is_failed()
		. has_message("Expecting:\n 'Warning aaa!'\n but was\n 'Warning xyz!'.")
	)
	(
		assert_failure(
			func() -> void:
				assert_result(GdUnitResult.error("Error 410")).contains_message("Error 500")
		)
		. is_failed()
		. has_message("Expecting:\n 'Error 500'\n but was\n 'Error 410'.")
	)
...
func test_is_value() -> void:
	...
	(
		assert_failure(
			func() -> void: assert_result(GdUnitResult.success(result_value)).is_value("")
		)
		. is_failed()
		. has_message("Expecting to contain same value:\n '<empty>'\n but was\n <Node>.")
	)
...
func test_override_failure_message() -> void:
	(
		assert_object(assert_result(GdUnitResult.success("")).override_failure_message("error"))
		. is_instanceof(GdUnitResultAssert)
	)
...
func test_append_failure_message() -> void:
	(
		assert_object(assert_result(GdUnitResult.success("")).append_failure_message("error"))
		. is_instanceof(GdUnitResultAssert)
	)
	@warning_ignore("unsafe_method_access")
	(

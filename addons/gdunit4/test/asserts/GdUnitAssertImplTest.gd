...
func test_get_line_number_multiline() -> void:
	...
	(
		assert_failure(func() -> void: assert_int(10).is_not_negative().is_equal(42))
		. is_failed()
		. has_line(58)
		. has_message("Expecting:\n '42'\n but was\n '10'")
	)
...
func test_is_null() -> void:
	...
	(
		...
		. has_line(78)
		. starts_with_message(
			"Expecting: '<null>' but was 'Color$v0'".replace("$v0", str(Color.RED))
		)
	)
...
func test_override_failure_message() -> void:
	...
	(
		assert_failure(
			func() -> void:
				assert_that(Color.RED).override_failure_message("Custom failure message").is_null()
		)
		. is_failed()
		. has_line(123)
...
func test_append_failure_message() -> void:
	assert_object(assert_that(null).append_failure_message("error")).is_instanceof(
		GdUnitObjectAssert
	)
	@warning_ignore("unsafe_method_access")
	(
		assert_failure(
			func() -> void:
				assert_that(null).append_failure_message("custom failure data").is_not_null()
		)
		. is_failed()
		. has_message(
			(

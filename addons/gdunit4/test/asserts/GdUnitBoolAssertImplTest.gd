...
func test_override_failure_message() -> void:
	assert_object(assert_bool(true).override_failure_message("error")).is_instanceof(
		GdUnitBoolAssert
	)
	(
		assert_failure(
			func() -> void:
				assert_bool(true).override_failure_message("Custom failure message").is_null()
		)
		. is_failed()
...
func test_append_failure_message() -> void:
	...
	(
		assert_failure(
			func() -> void:
				assert_bool(true).append_failure_message("custom failure data").is_false()
		)
		. is_failed()
		. has_message(
			(

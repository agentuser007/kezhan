...
func test_is_success() -> void:
	...
	(
		assert_failure(func() -> void: assert_error(produce_push_warning).is_success())
		. is_failed()
		. has_message(
			(
				"""
	...
	(
		assert_failure(func() -> void: assert_error(produce_push_error).is_success())
		. is_failed()
		. has_message(
			(
				"""
...
func test_is_assert_failed(
	...
	assert_error(produce_assert_error).is_runtime_error("Assertion failed: this is an assert error")

	(
		assert_failure(
			func() -> void:
				assert_error(produce_assert_success).is_runtime_error(
					"Assertion failed: this is an assert error"
				)
		)
		. is_failed()
		. has_message(
...
func test_is_push_warning() -> void:
	...
	(
		assert_failure(
			func() -> void: assert_error(produce_push_warning).is_push_warning("this is an error")
		)
		. is_failed()
		. has_message(
			(
...
func test_is_push_warning_using_argument_matcher() -> void:
	...
	(
		assert_failure(
			func() -> void: assert_error(produce_push_warning).is_push_warning(any_int())
		)
		. is_failed()
		. has_message("Only 'any()' and 'any_string()' argument matchers are allowed!")
	)
...
func test_is_push_error_using_argument_matcher() -> void:
	...
	(
		assert_failure(func() -> void: assert_error(produce_push_error).is_push_error(any_int()))
		. is_failed()
		. has_message("Only 'any()' and 'any_string()' argument matchers are allowed!")
	)
...
func test_is_runtime_error_using_argument_matcher(
	...
	(
		assert_failure(
			func() -> void: assert_error(produce_runtime_error).is_runtime_error(any_int())
		)
		. is_failed()
		. has_message("Only 'any()' and 'any_string()' argument matchers are allowed!")
	)

...
func test_is_equal(
	...
	_test_parameters := [
		...
		[
			"PackedColorArray",
			PackedColorArray([Color.RED, Color.GREEN, Color.BLUE, Color.YELLOW, Color.BLACK])
		]
	]
) -> void:
	...
	(
		...
		. has_message(
			...
			Index	Current	Expected	5	<N/A>	$value	"""
					. dedent()
					. trim_prefix("\n")
					. replace("$value", str(array[2]))
				)
				% [GdArrayTools.as_string(other, false), GdArrayTools.as_string(array, false)]
			)
		)
	)
...
func test_override_failure_message(
	_test: String,
	array: Variant,
	_test_parameters := [
		["Array", Array()],
	...
	assert_object(assert_array(array).override_failure_message("error")).is_instanceof(
		GdUnitArrayAssert
	)
	(
		assert_failure(
			func() -> void:
				assert_array(array).override_failure_message("Custom failure message").is_null()
		)
		. is_failed()

...
func test_is_null(timeout := 2000) -> void:
	...
	(
		(await assert_failure_await(
			func() -> void:
				await assert_func(value_provider, "obj_value", []).wait_until(100).is_null()
		))
...
func test_is_not_null(timeout := 2000) -> void:
	...
	(
		(await assert_failure_await(
			func() -> void:
				await assert_func(value_provider, "obj_value", []).wait_until(100).is_not_null()
		))
...
func test_is_true(timeout := 2000) -> void:
	...
	(
		(await assert_failure_await(
			func() -> void:
				await assert_func(value_provider, "bool_value", []).wait_until(100).is_true()
		))
...
func test_is_false(timeout := 2000) -> void:
	...
	(
		(await assert_failure_await(
			func() -> void:
				await assert_func(value_provider, "bool_value", []).wait_until(100).is_false()
		))
...
func test_is_equal(timeout := 2000) -> void:
	...
	(
		(await assert_failure_await(
			func() -> void:
				await assert_func(value_provider, "int_value", []).wait_until(100).is_equal(25)
		))
...
func test_is_not_equal(timeout := 2000) -> void:
	...
	(
		(await assert_failure_await(
			func() -> void:
				await assert_func(value_provider, "int_value", []).wait_until(100).is_not_equal(23)
		))
...
func test_timer_yielded_function() -> void:
	...
	(
		(await assert_failure_await(
			func() -> void:
				await assert_func(self, "timed_function", []).wait_until(100).is_equal(Color.RED)
		))
		. has_message(
			"Expected: is equal 'Color$v0' but timed out after 100ms".replace("$v0", str(Color.RED))
		)
	)
...
func test_timer_yielded_function_timeout() -> void:
	(
		(await assert_failure_await(
			func() -> void:
				await assert_func(self, "timed_function", []).wait_until(40).is_equal(Color.BLACK)
		))
...
func test_idle_frame_yielded_function() -> void:
	...
	(
		(await assert_failure_await(
			func() -> void:
				await assert_func(self, "yielded_function", []).wait_until(500).is_equal(Color.RED)
		))
		. has_message(
			"Expected: is equal 'Color$v0' but timed out after 500ms".replace("$v0", str(Color.RED))
		)
	)
...
func test_has_failure_message() -> void:
	...
	(
		(await assert_failure_await(
			func() -> void:
				await assert_func(value_provider, "int_value", []).wait_until(500).is_equal(42)
		))
...
func test_override_failure_message() -> void:
	(
		assert_object(
			assert_func(RefCounted.new(), "get_reference_count").override_failure_message("error")
		)
		. is_instanceof(GdUnitFuncAssert)
	)
	...
	(
		(await assert_failure_await(
			func() -> void:
				await (
					assert_func(value_provider, "int_value", [])
...
func test_append_failure_message() -> void:
	(
		assert_object(
			assert_func(RefCounted.new(), "get_reference_count").append_failure_message("error")
		)
		. is_instanceof(GdUnitFuncAssert)
	)
	(
		(await assert_failure_await(
			func() -> void:
				await (
					assert_func(RefCounted.new(), "get_reference_count")
...
func test_invalid_function(timeout := 100) -> void:
	(
		(await assert_failure_await(
			func() -> void:
				await assert_func(self, "invalid_func_name", []).wait_until(1000).is_equal(42)
		))

...
func test_override_failure_message() -> void:
	assert_object(assert_float(3.14).override_failure_message("error")).is_instanceof(
		GdUnitFloatAssert
	)
	...
	(
		assert_failure(
			func() -> void:
				assert_float(3.14).override_failure_message("Custom failure message").is_null()
		)
		. is_failed()
...
func test_append_failure_message() -> void:
	assert_object(assert_float(3.14).append_failure_message("error")).is_instanceof(
		GdUnitFloatAssert
	)
	@warning_ignore("unsafe_method_access")
	(
		assert_failure(
			func() -> void:
				assert_float(3.14).append_failure_message("custom failure data").is_zero()
		)
		. is_failed()
		. has_message(
			(
...
func test_is_failure() -> void:
	...
	assert_failure(func() -> void: assert_float(1.0).is_zero()).is_failed()

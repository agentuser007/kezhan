...
func test_invalid_arg() -> void:
	(
		(await assert_failure_await(
			func() -> void:
				await assert_signal(null).wait_until(50).is_emitted("test_signal_counted")
		))
	...
	(
		(await assert_failure_await(
			func() -> void:
				await assert_signal(null).wait_until(50).is_not_emitted("test_signal_counted")
		))
		. has_message("Can't wait for signal checked a NULL object.")
	)
...
func test_unknown_signal() -> void:
	(
		(await assert_failure_await(
			func() -> void: await assert_signal(signal_emitter).wait_until(50).is_emitted("unknown")
		))
...
func test_signal_is_emitted_by_invalid_type() -> void:
	(
		(await assert_failure_await(
			func() -> void: await assert_signal(signal_emitter).is_emitted(10)
		))
...
func test_signal_is_emitted_without_args() -> void:
	...
	(
		(await assert_failure_await(
			func() -> void:
				await assert_signal(signal_emitter).wait_until(500).is_emitted("test_signal_unused")
		))
...
func test_signal_is_emitted_with_array_args() -> void:
	...
	(
		(await assert_failure_await(
			func() -> void:
				await assert_signal(signal_emitter).wait_until(50).is_emitted(
					"test_signal_counted", [500]
				)
		))
		. has_message(
			"Expecting emit signal: 'test_signal_counted([500])' but timed out after 50ms"
		)
	)
...
func test_signal_is_emitted_use_argument_matcher() -> void:
	...
	(
		(await assert_failure_await(
			func() -> void:
				await assert_signal(signal_emitter).wait_until(50).is_emitted(
					"test_signal_counted", [any_string()]
				)
		))
		. has_message(
			"Expecting emit signal: 'test_signal_counted([any_string()])' but timed out after 50ms"
		)
	)
...
func test_signal_is_not_emitted_by_signal_type() -> void:
	await assert_signal(signal_emitter).wait_until(50).is_not_emitted(
		signal_emitter.test_signal_counted
	)
	await assert_signal(signal_emitter).wait_until(30).is_not_emitted(
		signal_emitter.test_signal_counted, 50
	)
...
func test_signal_is_not_emitted() -> void:
	...
	(
		(await assert_failure_await(
			func() -> void:
				await assert_signal(signal_emitter).wait_until(1000).is_not_emitted(
					"test_signal_counted", [50]
				)
		))
		. starts_with_message(
			"Expecting do not emit signal: 'test_signal_counted([50])' but is emitted after"
		)
	)
...
func test_signal_is_not_emitted_use_argument_matcher() -> void:
	...
	await assert_signal(signal_emitter).wait_until(10).is_not_emitted(
		"test_signal_counted", [any()]
	)
	await assert_signal(signal_emitter).wait_until(10).is_not_emitted(
		"test_signal_counted", [any_int()]
	)
	...
	(
		...
		. starts_with_message(
			"Expecting do not emit signal: 'test_signal_counted([any()])' but is emitted after"
		)
	)
...
func test_override_failure_message() -> void:
	assert_object(assert_signal(signal_emitter).override_failure_message("error")).is_instanceof(
		GdUnitSignalAssert
	)
	(
		(await assert_failure_await(
...
func test_append_failure_message() -> void:
	assert_object(assert_signal(signal_emitter).append_failure_message("error")).is_instanceof(
		GdUnitSignalAssert
	)
	(
		(await assert_failure_await(
...
func test_is_signal_exists() -> void:
	...
	(
		(await assert_failure_await(
			func() -> void: assert_signal(node).is_signal_exists("not_existing_signal")
		))

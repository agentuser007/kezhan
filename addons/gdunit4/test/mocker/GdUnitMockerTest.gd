...
func test_is_mockable_godot_classes() -> void:
	...
	for clazz_name in ClassDB.get_class_list():
		...
		var is_mockable: bool = (
			not Engine.has_singleton(clazz_name)
			and ClassDB.can_instantiate(clazz_name)
			and clazz_name.find("_") != 0
		)
		(
...
func test_is_mockable_by_script_path() -> void:
	(
		assert_bool(GdUnitMockBuilder.is_mockable(resource_path + "CustomResourceTestClass.gd"))
		. is_true()
	)
...
func test_is_mockable__overriden_func_get_class() -> void:
	...
	(
		...
		. override_failure_message(
			"The class 'CustomResourceTestClass' should be mockable when 'func get_class()' is overriden"
		)
		. is_true()
	)
...
func test_mock_godot_class_fullcheck(
	fuzzer := GodotClassNameFuzzer.new(), _fuzzer_iterations := 200
) -> void:
...
func test_mock_source_with_class_name_by_resource_path() -> void:
	...
	assert_str(head).contains("class_name DoubledMockClassMunderwoodPathingWorld").contains(
		"extends '%s'" % resource_path_
	)
...
func test_mock_extends_godot_class() -> void:
	...
	var head: String = m.get_script().source_code.substr(0, 500)
	assert_str(head).contains("class_name DoubledMockClassWorld").contains("extends World3D")
...
func test_mock_void_func_not_allowed() -> void:
	...
	(
		assert_error(func() -> void: do_return("overridden value").on(m).foo_void())
...
func test_mock_custom_class_func_return_type_internal_class_enum() -> void:
	...
	assert_that(m.get_inner_class_enum()).is_equal(
		ClassWithEnumReturnTypes.InnerClass.TEST_ENUM.FOO
	)
	do_return(ClassWithEnumReturnTypes.InnerClass.TEST_ENUM.BAR).on(m).get_inner_class_enum()
	assert_that(m.get_inner_class_enum()).is_equal(
		ClassWithEnumReturnTypes.InnerClass.TEST_ENUM.BAR
	)
	...
	# verify enum return type
	assert_that(m2.get_inner_class_enum()).is_equal(
		ClassWithEnumReturnTypes.InnerClass.TEST_ENUM.FOO
	)
...
func test_mock_snake_case_named_class_by_resource_path() -> void:
	...
	var mock_b: Variant = mock(
		"res://addons/gdUnit4/test/mocker/resources/snake_case_class_name.gd"
	)
	assert_object(mock_b).is_not_null()
...
func test_mock_scene_variables_is_set() -> void:
	...
	@warning_ignore("unsafe_cast")
	assert_bool(
		mocked_scene.is_connected(
			"panel_color_change", Callable(mocked_scene as Object, "_on_panel_color_changed")
		)
	)

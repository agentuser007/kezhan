...
func test_mock_on_script_path_without_class_name() -> void:
	var instance: Object = (
		(load("res://addons/gdUnit4/test/mocker/resources/ClassWithoutNameA.gd") as GDScript).new()
	)
	var script := GdUnitMockBuilder.mock_on_script(
		instance, "res://addons/gdUnit4/test/mocker/resources/ClassWithoutNameA.gd", [], true
	)
...
func test_mock_on_script_path_with_custom_class_name() -> void:
	...
	var instance: Object = (
		(load("res://addons/gdUnit4/test/mocker/resources/ClassWithCustomClassName.gd") as GDScript)
		. new()
	)
	var script := GdUnitMockBuilder.mock_on_script(
		instance,
		"res://addons/gdUnit4/test/mocker/resources/ClassWithCustomClassName.gd",
		[],
		false
	)
...
func test_mock_on_class_with_custom_class_name() -> void:
	# the class contains a class_name definition
	var script := GdUnitMockBuilder.mock_on_script(
		GdUnit_Test_CustomClassName.new(), GdUnit_Test_CustomClassName, [], false
	)
	assert_str(script.resource_name).starts_with("MockGdUnitTestCustomClassName")
	assert_that(script.get_instance_base_type()).is_equal("Resource")
	# finally check the mocked script is valid
...
func test_mock_on_script_with_multilines() -> void:
	var instance: Object = (
		(load("res://addons/gdUnit4/test/mocker/resources/ClassWithMultilineBlocks.gd") as GDScript)
		. new()
	)

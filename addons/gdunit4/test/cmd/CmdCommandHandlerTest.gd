...
func test_register_cb() -> void:
	...
	assert_dict(cmd_handler._command_cbs).contains_key_value(
		"-a", [_cmd_instance.cmd_arg, CmdCommandHandler.NO_CB]
	)
...
func test_register_cbv() -> void:
	...
	assert_dict(cmd_handler._command_cbs).contains_key_value(
		"-a", [CmdCommandHandler.NO_CB, _cmd_instance.cmd_args]
	)
...
func test_register_cb_and_cbv() -> void:
	...
	cmd_handler.register_cbv("-a", _cmd_instance.cmd_args)
	assert_dict(cmd_handler._command_cbs).contains_key_value(
		"-a", [_cmd_instance.cmd_arg, _cmd_instance.cmd_args]
	)
...
func test__validate_registerd_unknown_commands() -> void:
	...
	(
		...
		. is_error()
		. contains_message(
			"The command '-d' is unknown, verify your CmdOptions!\nThe command '-y' is unknown, verify your CmdOptions!"
		)
	)
...
func test__validate_registerd_invalid_callbacks() -> void:
	...
	assert_result(cmd_handler._validate()).is_error().contains_message(
		"Invalid function reference for command '-arg', Check the function reference!"
	)
...
func test__validate_registerd_register_same_callback_twice() -> void:
	...
	assert_result(cmd_handler._validate()).is_error().contains_message(
		"The function reference 'cmd_no_arg' already registerd for command '-a'!"
	)
		...
		assert_result(
			cmd_handler.execute(
				[
					...
					CmdCommand.new("-arg", ["value1", "value2"])
				]
			)
		)
		. is_success()

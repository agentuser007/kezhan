...
func before(
	do_skip := not GdUnit4CSharpApiLoader.is_api_loaded(),
	skip_reason := "Do run only for Godot .Net version"
) -> void:
	pass


@warning_ignore("unused_parameter")
func test_is_engine_version_supported(
	version: int,
	expected: bool,
	test_parameters := [
		[0x40101, false], [0x40102, false], [0x40100, false], [0x40300, true], [0x40400, true]
	]
) -> void:
...
func test_create_test_suite() -> void:
	...
	var result := GdUnitFileAccess.copy_file(
		"res://addons/gdUnit4/test/resources/core/sources/TestPerson.cs", temp
	)
	assert_result(result).is_success()

	var example_source_cs := result.value_as_string()
	var source := load(example_source_cs)
	var test_suite_path := GdUnitTestSuiteScanner.resolve_test_suite_path(
		source.resource_path, "test"
	)
...
func test_discover_tests() -> void:
	...
	assert_array(tests).has_size(14).contains([any_class(GdUnitTestCase)])

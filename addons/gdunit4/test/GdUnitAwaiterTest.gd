...
func install_signal_emitter(
	signal_name: String, signal_args: Array = [], time_out: float = 0.020
) -> void:
	var timer := Timer.new()
	add_child(timer)
...
func emit_test_signal(signal_name: String, signal_args: Array) -> void:
	match signal_args.size():
		...
		1:
			emit_signal(signal_name, signal_args[0])
		2:
			emit_signal(signal_name, signal_args[0], signal_args[1])
		3:
			emit_signal(signal_name, signal_args[0], signal_args[1], signal_args[2])
...
func test_await_signal_on_never_emitted_timedout() -> void:
	(
		...
		(await assert_failure_await(
			func x() -> void: await await_signal_on(self, "test_signal_c", ["yyy"], 200)
		))
		. has_line(72)
...
func test_await_signal_on_invalid_source_timedout() -> void:
	(
		...
		(await assert_failure_await(
			func x() -> void: await await_signal_on(invalid_node(), "tree_entered", [], 300)
		))
		. has_line(80)
		. has_message(
			GdAssertMessages.error_await_signal_on_invalid_instance(null, "tree_entered", [])
		)
	)

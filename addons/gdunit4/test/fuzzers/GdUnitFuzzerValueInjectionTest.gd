	...
	for test_case: String in _expected_iterations.keys():
		...
		(
			...
			. override_failure_message(
				(
					"Expecting %s itertions but is %s checked test case %s"
					% [expected, current, test_case]
				)
			)
			. is_equal(expected)
		)
...
func test_fuzzer_has_same_instance_peer_iteration(
	fuzzer := TestFuzzer.new(), _fuzzer_iterations := 10
) -> void:
	...
	if _fuzzer_instance_before != null:
		assert_object(fuzzer).is_same(_fuzzer_instance_before)
...
func test_fuzzer_iterations_custom_value(
	_fuzzer := Fuzzers.rangei(-23, 22), _fuzzer_iterations := 234, _fuzzer_seed := 100
) -> void:
	_current_iterations["test_fuzzer_iterations_custom_value"] += 1


func test_fuzzer_inject_value(fuzzer := Fuzzers.rangei(-23, 22), _fuzzer_iterations := 100) -> void:
	_current_iterations["test_fuzzer_inject_value"] += 1
	assert_object(fuzzer).is_not_null()
	assert_int(fuzzer.next_value()).is_between(-23, 22)


func test_fuzzer_with_timeout(
	fuzzer := Fuzzers.rangei(-23, 22), _fuzzer_iterations := 20, _timeout := 100
) -> void:
...
var expected_value: Array[int] = [22, 3, -14, -16, 21, 20, 4, -23, -19, -5]


func test_fuzzer_inject_value_with_seed(
	fuzzer := Fuzzers.rangei(-23, 22), _fuzzer_iterations := 10, _fuzzer_seed := 187772
) -> void:
	assert_object(fuzzer).is_not_null()
	...
	(
		...
		. override_failure_message(
			(
				...
				% [expected, iteration_index, current]
			)
		)
		. is_equal(expected)
	)
...
var expected_value_a: Array[int] = [22, -14, 21, 4, -19, -11, 5, 21, -6, -9]
var expected_value_b: Array[int] = [35, 38, 34, 39, 35, 41, 37, 35, 34, 39]


func test_multiple_fuzzers_inject_value_with_seed(
	fuzzer_a := Fuzzers.rangei(-23, 22),
	fuzzer_b := Fuzzers.rangei(33, 44),
	...
	(
		...
		. override_failure_message(
			(
				"Expect value %s checked test iteration %s\n but was %s"
				% [expected_a, iteration_index_a, current_a]
			)
		)
		. is_equal(expected_a)
	)
	...
	(
		...
		. override_failure_message(
			(
				"Expect value %s checked test iteration %s\n but was %s"
				% [expected_b, iteration_index_b, current_b]
			)
		)
		. is_equal(expected_b)
	)
...
func test_fuzzer_error_after_eight_iterations(
	fuzzer := TestFuzzer.new(), _fuzzer_iterations := 10
) -> void:
	assert_object(fuzzer).is_not_null()
...
func test_fuzzer_custom_func(fuzzer := get_fuzzer()) -> void:
	assert_object(fuzzer).is_not_null()
...
func test_fuzzing_with_untyped_parameters(
	float_fuzzer = Fuzzers.rangef(-100.0, 100.0), _fuzzer_iterations = 10
):
	assert_float(float_fuzzer.next_value()).is_between(-100.0, 100.0)


func test_boolean_fuzzer(bool_fuzzer := Fuzzers.boolean(), _fuzzer_iterations := 50) -> void:
	var value := bool_fuzzer.next_value()
	assert_bool(value in [true, false]).is_true()

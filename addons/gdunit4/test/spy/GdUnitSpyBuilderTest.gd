...
func test_spy_on_script_respect_virtual_functions() -> void:
	var do_spy: Variant = auto_free(
		GdUnitSpyBuilder.spy_on_script(auto_free(NodeWithOutVirtualFunc.new()), [], true).new()
	)
...
func test_spy_on_scene_with_onready_parameters() -> void:
	...
	var scene: TestSceneWithProperties = (
		load("res://addons/gdUnit4/test/spy/resources/TestSceneWithProperties.tscn").instantiate()
	)
	add_child(scene)

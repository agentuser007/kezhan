...
func test_spy_on_Node() -> void:
	...
	assert_object(spy_node).is_not_null().is_instanceof(Node).is_not_same(instance)
...
func test_spy_source_with_class_name_by_resource_path() -> void:
	var instance: Object = auto_free(
		_load("res://addons/gdUnit4/test/mocker/resources/GD-256/world.gd").new()
	)
	var m: Variant = spy(instance)
	@warning_ignore("unsafe_method_access")
	var head: String = m.get_script().source_code.substr(0, 500)
	assert_str(head).contains("class_name DoubledSpyClassMunderwoodPathingWorld").contains(
		"extends 'res://addons/gdUnit4/test/mocker/resources/GD-256/world.gd'"
	)
...
func test_spy_extends_godot_class() -> void:
	...
	@warning_ignore("unsafe_method_access")
	var head: String = m.get_script().source_code.substr(0, 500)
	assert_str(head).contains("class_name DoubledSpyClassWorld").contains("extends World3D")
...
func test_spy_on_custom_class() -> void:
	...
	assert_object(spy_instance).is_not_null().is_instanceof(AdvancedTestClass).is_not_same(instance)
...
func test_spy_class_with_custom_formattings() -> void:
	var resource := _load(
		"res://addons/gdUnit4/test/mocker/resources/ClassWithCustomFormattings.gd"
	)
	...
	assert_failure(func() -> void: verify_no_interactions(do_spy)).is_failed().has_line(140)


func test_spy_copied_class_members() -> void:
	var instance: TestPerson = auto_free(
		_load("res://addons/gdUnit4/test/mocker/resources/TestPerson.gd").new(
			"user-x", "street", 56616
		)
	)
...
func test_spy_on_inner_class() -> void:
	var instance: AdvancedTestClass.AtmosphereData = auto_free(
		AdvancedTestClass.AtmosphereData.new()
	)
...
func test_spy_snake_case_named_class_by_resource_path() -> void:
	...
	var instance_b: Object = (
		_load("res://addons/gdUnit4/test/mocker/resources/snake_case_class_name.gd").new()
	)
...
func test_spy_scene_by_resource_path() -> void:
	...
	assert_object(spy_scene).is_not_null().is_not_instanceof(PackedScene).is_instanceof(Control)
	@warning_ignore("unsafe_method_access")
...
func test_spy_on_PackedScene() -> void:
	...
	assert_object(spy_scene).is_not_null().is_not_instanceof(PackedScene).is_not_same(resource)
	@warning_ignore("unsafe_method_access")
	assert_object(spy_scene.get_script()).is_not_null().is_instanceof(GDScript).is_not_same(
		original_script
	)
	@warning_ignore("unsafe_method_access")
	assert_str(spy_scene.get_script().resource_name).is_equal("SpyTestScene.gd")
...
func test_spy_scene_by_instance() -> void:
	...
	assert_object(spy_scene).is_not_null().is_same(instance).is_instanceof(Control)
	@warning_ignore("unsafe_method_access")
	assert_object(spy_scene.get_script()).is_not_null().is_instanceof(GDScript).is_not_same(
		original_script
	)
...
func test_spy_scene_by_path_fail_has_no_script_attached() -> void:
	var resource: PackedScene = load(
		"res://addons/gdUnit4/test/mocker/resources/scenes/TestSceneWithoutScript.tscn"
	)
...
func test_spy_scene_initalize() -> void:
	...
	assert_bool(
		spy_scene.is_connected(
			"panel_color_change", Callable(spy_scene as Node, "_on_panel_color_changed")
		)
	)

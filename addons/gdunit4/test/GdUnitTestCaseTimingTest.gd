	...
	for test_case: String in _test_values_expected.keys():
		...
		(
			...
			. override_failure_message(
				(
					"Expect before_test called %s times but is %s for test case %s"
					% [expected._test_before_calls, current._test_before_calls, test_case]
				)
			)
		...
		(
			...
			. override_failure_message(
				(
					"Expect after_test called %s times but is %s for test case %s"
					% [expected._test_before_calls, current._test_before_calls, test_case]
				)
			)
...
func test_multi_yielding_with_fuzzer(
	fuzzer := Fuzzers.rangei(0, 1000), fuzzer_iterations := 5
) -> void:
	...
	if fuzzer.iteration_index() == 5:
		...
		assert_int(Time.get_ticks_msec() - _iteration_timer_start).is_greater_equal(
			3900 * fuzzer_iterations
		)

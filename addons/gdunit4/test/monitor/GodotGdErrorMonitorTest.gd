...
func test_monitor_push_error() -> void:
	...
	assert_str(report.message()).contains("Test GodotGdErrorMonitor 'push_error' reporting")
	assert_object(report.stack_trace()).is_equal(
		GdUnitStackTrace.new(
			[
				GdUnitStackTraceElement.new(
					"res://addons/gdUnit4/test/monitor/GodotGdErrorMonitorTest.gd",
					74,
					"force_push_error2"
				),
				GdUnitStackTraceElement.new(
...
func test_monitor_push_waring() -> void:
	...
	assert_array(reports).has_size(1)
	var report := reports[0]
	assert_str(report.message()).contains("Test GodotGdErrorMonitor 'push_warning' reporting")
	assert_object(report.stack_trace()).is_equal(
		GdUnitStackTrace.new(
			[
				GdUnitStackTraceElement.new(
...
func test_fail_by_push_error(
	_do_skip := true,
	_skip_reason := "disabled to not produce errors, enable only for direct testing"
) -> void:

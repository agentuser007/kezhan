...
static func build(
	template: String, report: GdUnitReportSummary, report_link: String, breadcrumb_path_link := ""
) -> String:
	return (
		template
...
static func create_test_failure_report(_report_dir: String, report: GdUnitTestCaseReport) -> String:
	return (
		...
		. replace(
			GdUnitHtmlPatterns.FAILURE_REPORT, GdUnitHtmlEncoder.encode(report.failure_report())
		)
	)

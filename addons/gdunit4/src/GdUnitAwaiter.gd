...
func await_signal_on(
	source: Object, signal_name: String, args: Array = [], timeout_millis: int = 2000
) -> Variant:
	...
	var line_number := stack_trace.get_line_number()
	if not is_instance_valid(source):
		@warning_ignore("return_value_discarded")
		assert_that.report_error(
			GdAssertMessages.error_await_signal_on_invalid_instance(source, signal_name, args),
			line_number
		)
	...
	if awaiter.is_interrupted():
		var failure := (
			"await_signal_on(%s, %s) timed out after %sms" % [signal_name, args, timeout_millis]
		)
		@warning_ignore("return_value_discarded")
...
# args: the expected signal arguments as an array
# timeout: the timeout in ms, default is set to 2000ms
func await_signal_idle_frames(
	source: Object, signal_name: String, args: Array = [], timeout_millis: int = 2000
) -> Variant:
	var line_number := GdUnitStackTrace.new().get_line_number()
	# fail fast if the given source instance invalid
	if not is_instance_valid(source):
		@warning_ignore("return_value_discarded")
		GdUnitAssertImpl.new(signal_name).report_error(
			GdAssertMessages.error_await_signal_on_invalid_instance(source, signal_name, args),
			line_number
		)
		return await await_idle_frame()
	...
	if awaiter.is_interrupted():
		var failure := (
			"await_signal_idle_frames(%s, %s) timed out after %sms"
			% [signal_name, args, timeout_millis]
		)
		@warning_ignore("return_value_discarded")

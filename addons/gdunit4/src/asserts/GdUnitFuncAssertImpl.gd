...
func _init(instance: Object, func_name: String, args := Array()) -> void:
	...
	if not instance.has_method(func_name):
		...
		report_error(
			"The function '%s' do not exists checked instance '%s'." % [func_name, instance]
		)
		_interrupted = true
	else:
...
func wait_until(timeout := 2000) -> GdUnitFuncAssert:
	if timeout <= 0:
		push_warning(
			"Invalid timeout param, alloed timeouts must be grater than 0. Use default timeout instead"
		)
...
func cb_is_not_null(c: Variant, _e: Variant) -> bool:
	return c != null


func cb_is_false(c: Variant, _e: Variant) -> bool:
	return c == false


func cb_is_true(c: Variant, _e: Variant) -> bool:
	return c == true


func cb_is_equal(c: Variant, e: Variant) -> bool:
...
func _validate_callback(predicate: Callable, expected: Variant = null) -> void:
	...
	if _interrupted:
		...
		@warning_ignore("return_value_discarded")
		report_error(
			GdAssertMessages.error_interrupted(

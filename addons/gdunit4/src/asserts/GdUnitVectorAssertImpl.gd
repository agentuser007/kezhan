...
func _init(current: Variant, type_check := true) -> void:
	...
	if not _validate_value_type(current):
		...
		report_error(
			(
				"GdUnitVectorAssert error, the type <%s> is not supported."
...
func is_equal_approx(expected: Variant, approx: Variant) -> GdUnitVectorAssert:
	...
	if current == null or (not _is_equal_approx(current, from, to)):
		return report_error(
			GdAssertMessages.error_is_value(Comparator.BETWEEN_EQUAL, current, from, to)
		)
...
func _is_equal_approx(current: Variant, from: Variant, to: Variant) -> bool:
	match typeof(current):
		TYPE_VECTOR2, TYPE_VECTOR2I:
			return (
				(current.x >= from.x and current.y >= from.y)
				and (current.x <= to.x and current.y <= to.y)
			)
		TYPE_VECTOR3, TYPE_VECTOR3I:
			return (
				(current.x >= from.x and current.y >= from.y and current.z >= from.z)
		...
		TYPE_VECTOR4, TYPE_VECTOR4I:
			return (
				(
					current.x >= from.x
					and current.y >= from.y
					and current.z >= from.z
					and current.w >= from.w
				)
				and (
					current.x <= to.x
					and current.y <= to.y
		...
		_:
			push_error(
				"Missing implementation '_is_equal_approx' for vector type %s" % typeof(current)
			)
			return false
...
func is_less(expected: Variant) -> GdUnitVectorAssert:
	...
	if current == null or current >= expected:
		return report_error(
			GdAssertMessages.error_is_value(Comparator.LESS_THAN, current, expected)
		)
	return report_success()
...
func is_less_equal(expected: Variant) -> GdUnitVectorAssert:
	...
	if current == null or current > expected:
		return report_error(
			GdAssertMessages.error_is_value(Comparator.LESS_EQUAL, current, expected)
		)
	return report_success()
...
func is_greater(expected: Variant) -> GdUnitVectorAssert:
	...
	if current == null or current <= expected:
		return report_error(
			GdAssertMessages.error_is_value(Comparator.GREATER_THAN, current, expected)
		)
	return report_success()
...
func is_greater_equal(expected: Variant) -> GdUnitVectorAssert:
	...
	if current == null or current < expected:
		return report_error(
			GdAssertMessages.error_is_value(Comparator.GREATER_EQUAL, current, expected)
		)
	return report_success()
...
func is_not_between(from: Variant, to: Variant) -> GdUnitVectorAssert:
	...
	if current != null and current >= from and current <= to:
		return report_error(
			GdAssertMessages.error_is_value(Comparator.NOT_BETWEEN_EQUAL, current, from, to)
		)
	return report_success()

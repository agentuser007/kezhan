...
func report_error(failure: String) -> GdUnitAssert:
	var failure_message := GdAssertMessages.build_failure_message(
		failure, _additional_failure_message, _custom_failure_message
	)
	GdAssertReports.report_error(
		GdUnitError.new(failure_message, _line_number, GdUnitStackTrace.new())
	)
	return self
...
func wait_until(timeout := 2000) -> GdUnitSignalAssert:
	if timeout <= 0:
		...
		report_warning(
			"Invalid timeout parameter, allowed timeouts must be greater than 0, use default timeout instead!"
		)
...
func is_signal_exists(signal_or_name: Variant) -> GdUnitSignalAssert:
	if not (signal_or_name is String or signal_or_name is Signal):
		return report_error(
			(
				"Invalid signal_name: expected String or Signal, but is '%s'"
				% type_string(typeof(signal_or_name))
			)
		)
	...
	if not _emitter.has_signal(signal_name):
		@warning_ignore("return_value_discarded")
		report_error(
			"The signal '%s' not exists checked object '%s'." % [signal_name, _emitter.get_class()]
		)
	return self
...
func is_emitted(signal_name: Variant, ...signal_args: Array) -> GdUnitSignalAssert:
	...
	return await _wail_until_signal(signal_name, _wrap_arguments.callv(signal_args), false)
...
func is_not_emitted(signal_name: Variant, ...signal_args: Array) -> GdUnitSignalAssert:
	_line_number = GdUnitStackTrace.new().get_line_number()
	@warning_ignore("unsafe_call_argument")
	return await _wail_until_signal(signal_name, _wrap_arguments.callv(signal_args), true)
...
func _wail_until_signal(
	signal_or_name: Variant, expected_args: Array, expect_not_emitted: bool
) -> GdUnitSignalAssert:
	if _emitter == null:
	...
	if not _emitter.has_signal(signal_name):
		return report_error(
			(
				"Can't wait for non-existion signal '%s' checked object '%s'."
				% [signal_name, _emitter.get_class()]
			)
		)
	...
	while not _interrupted and not is_signal_emitted:
		...
		if is_instance_valid(_emitter):
			...
			if is_signal_emitted and expect_not_emitted:
				@warning_ignore("return_value_discarded")
				report_error(
					GdAssertMessages.error_signal_emitted(
						signal_name,
						expected_args,
						LocalTime.elapsed(int(_timeout - timer.time_left * 1000))
					)
				)
	...
	if _interrupted and not expect_not_emitted:
		@warning_ignore("return_value_discarded")
		report_error(
			GdAssertMessages.error_wait_signal(
				signal_name, expected_args, LocalTime.elapsed(_timeout)
			)
		)
	timer.free()
	if is_instance_valid(_emitter):

...
static func _index_report_as_table(index_reports: Array) -> String:
	...
	var cells := (
		header.replace("$text", "Index")
		+ header.replace("$text", "Current")
		+ header.replace("$text", "Expected")
	)
	for report: Variant in index_reports:
		...
		cells += (
			cell.replace("$text", index)
			+ cell.replace("$text", current)
			+ cell.replace("$text", expected)
		)
...
static func orphan_warning(orphans_count: int) -> String:
	return (
		...
		%s: Found %s possible orphan nodes.
			Add %s to the end of the test to collect details."""
			. dedent()
			. trim_prefix("\n")
		)
		% [
...
static func orphan_detected_on_suite_setup(orphans: Array[GdUnitOrphanNodeInfo]) -> String:
	return (
		...
		% [_warning("WARNING:"), _nerror(orphans.size()), _build_orphan_node_stacktrace(orphans)]
	)
...
static func test_timeout(timeout: int) -> String:
	return (
		...
		% [
			_error("Timeout !"),
			_colored_value("Test timed out after %s" % LocalTime.elapsed(timeout))
		]
	)
...
static func test_suite_skipped(hint: String, skip_count: int) -> String:
	return (
		...
		% [
			_error("The Entire test-suite is skipped!"),
			_colored_value(skip_count),
			_colored_value(hint)
		]
	)
...
static func error_is_null(current: Variant) -> String:
	return (
		"%s %s but was %s" % [_error("Expecting:"), _colored_value(null), _colored_value(current)]
	)
...
static func error_not_same_error(current: Variant, expected: Variant) -> String:
	return (
		"%s\n %s\n but was\n %s"
		% [_error("Expecting error message:"), _colored_value(expected), _colored_value(current)]
	)
...
static func error_is_instanceof(current: GdUnitResult, expected: GdUnitResult) -> String:
	return (
		"%s\n %s\n But it was %s"
		% [
			_error("Expected instance of:"),
			_colored_value(expected.or_else(null)),
			_colored_value(current.or_else(null))
		]
	)
...
static func error_is_false(current: Variant) -> String:
	return (
		"%s %s but is %s" % [_error("Expecting:"), _colored_value(false), _colored_value(current)]
	)
...
static func error_is_value(
	operation: int, current: Variant, expected: Variant, expected2: Variant = null
) -> String:
	match operation:
		Comparator.EQUAL:
			return (
				"%s\n %s but was '%s'"
				% [_error("Expecting:"), _colored_value(expected), _nerror(current)]
			)
		Comparator.LESS_THAN:
			return (
				"%s\n %s but was '%s'"
				% [_error("Expecting to be less than:"), _colored_value(expected), _nerror(current)]
			)
		Comparator.LESS_EQUAL:
			return (
				"%s\n %s but was '%s'"
				% [
					_error("Expecting to be less than or equal:"),
					_colored_value(expected),
					_nerror(current)
				]
			)
		Comparator.GREATER_THAN:
			return (
				"%s\n %s but was '%s'"
				% [
					_error("Expecting to be greater than:"),
					_colored_value(expected),
					_nerror(current)
				]
			)
		Comparator.GREATER_EQUAL:
			return (
				"%s\n %s but was '%s'"
				% [
					_error("Expecting to be greater than or equal:"),
					_colored_value(expected),
					_nerror(current)
				]
			)
		Comparator.BETWEEN_EQUAL:
			return (
				...
				% [
					_error("Expecting:"),
					_colored_value(current),
					_colored_value(expected),
					_colored_value(expected2)
				]
			)
...
static func error_is_in(current: Variant, expected: Array) -> String:
	return (
		"%s\n %s\n is in\n %s"
		% [_error("Expecting:"), _colored_value(current), _colored_value(str(expected))]
	)
...
static func error_contains(current: Variant, expected: Variant) -> String:
	return (
		"%s\n %s\n do contains\n %s"
		% [_error("Expecting:"), _colored_value(current), _colored_value(expected)]
	)
	...
	match compare_operator:
		Comparator.EQUAL:
			return (
				"%s\n %s but was '%s' in\n %s"
				% [
					_error("Expecting size:"),
					_colored_value(expected),
					_nerror(current_length),
					_colored_value(current)
				]
			)
		Comparator.LESS_THAN:
			return (
				"%s\n %s but was '%s' in\n %s"
				% [
					_error("Expecting size to be less than:"),
					_colored_value(expected),
					_nerror(current_length),
					_colored_value(current)
				]
		...
		Comparator.LESS_EQUAL:
			return (
				"%s\n %s but was '%s' in\n %s"
				% [
					_error("Expecting size to be less than or equal:"),
					_colored_value(expected),
					_nerror(current_length),
					_colored_value(current)
				]
		...
		Comparator.GREATER_THAN:
			return (
				"%s\n %s but was '%s' in\n %s"
				% [
					_error("Expecting size to be greater than:"),
					_colored_value(expected),
					_nerror(current_length),
					_colored_value(current)
				]
		...
		Comparator.GREATER_EQUAL:
			return (
				"%s\n %s but was '%s' in\n %s"
				% [
					_error("Expecting size to be greater than or equal:"),
					_colored_value(expected),
					_nerror(current_length),
					_colored_value(current)
				]
...
static func error_arr_contains(
	current: Variant, expected: Variant, not_expect: Variant, not_found: Variant, by_reference: bool
) -> String:
	var failure_message := (
		"Expecting contains SAME elements:" if by_reference else "Expecting contains elements:"
	)
	var error := (
...
static func error_arr_contains_exactly(
	...
	var failure_message := (
		...
		if compare_mode == GdObjects.COMPARE_MODE.PARAMETER_DEEP_TEST
...
static func error_arr_not_contains(
	current: Variant, expected: Variant, found: Variant, compare_mode: GdObjects.COMPARE_MODE
) -> String:
	var failure_message := (
		"Expecting:"
		if compare_mode == GdObjects.COMPARE_MODE.PARAMETER_DEEP_TEST
...
static func error_contains_keys(
	current: Array, expected: Array, keys_not_found: Array, compare_mode: GdObjects.COMPARE_MODE
) -> String:
	var failure := (
...
static func error_contains_key_value(
	key: Variant, value: Variant, current_value: Variant, compare_mode: GdObjects.COMPARE_MODE
) -> String:
	var failure := (
	...
	return (
		...
		% [
			_error(failure),
			_colored_value(key),
			_colored_value(value),
			_colored_value(key),
			_colored_value(current_value)
		]
	)
...
static func error_result_has_message_on_success(expected: String) -> String:
	return (
		...
		% [_error("Expecting:"), _colored_value(expected)]
	)
...
static func error_result_is_value(current: Variant, expected: Variant) -> String:
	return (
		"%s\n %s\n but was\n %s."
		% [
			_error("Expecting to contain same value:"),
			_colored_value(expected),
			_colored_value(current)
		]
	)
...
static func _result_error_message(current: GdUnitResult, expected_type: int) -> String:
	if current == null:
		return _error(
			"Expecting the result must be a %s but was <null>." % result_type(expected_type)
		)
	if current.is_success():
		return _error(
			"Expecting the result must be a %s but was SUCCESS." % result_type(expected_type)
		)
	var error := (
		"Expecting the result must be a %s but was %s:"
		% [result_type(expected_type), result_type(current._state)]
	)
...
static func error_interrupted(func_name: String, expected: Variant, elapsed: String) -> String:
	...
	return (
		"%s %s %s but timed out after %s"
		% [_error("Expected:"), func_name, _colored_value(expected), elapsed]
	)
...
static func error_wait_signal(signal_name: String, args: Array, elapsed: String) -> String:
	if args.is_empty():
		return (
			"%s %s but timed out after %s"
			% [_error("Expecting emit signal:"), _colored_value(signal_name + "()"), elapsed]
		)
	return (
		"%s %s but timed out after %s"
		% [
			_error("Expecting emit signal:"),
			_colored_value(signal_name + "(" + str(args) + ")"),
			elapsed
		]
	)
...
static func error_await_signal_on_invalid_instance(
	source: Variant, signal_name: String, args: Array
) -> String:
	return (
		"%s\n await_signal_on(%s, %s, %s)"
		% [
			_error("Invalid source! Can't await on signal:"),
			_colored_value(source),
			signal_name,
			args
		]
	)
...
static func error_no_more_interactions(summary: Dictionary) -> String:
	...
	return (
		"%s\n%s\n%s"
		% [
			_error("Expecting no more interactions!"),
			_error("But found interactions on:"),
			"\n".join(interactions)
		]
	)


static func error_validate_interactions(
	current_interactions: Dictionary, expected_interactions: Dictionary
) -> String:
	...
	return (
		"%s\n%s\n%s\n%s"
		% [
			_error("Expecting interaction on:"),
...
static func _find_first_diff(left: Array, right: Array) -> String:
	for index in left.size():
		...
		if not GdObjects.equals(l, r):
			return (
				"at position %s\n '%s' vs '%s'"
				% [_colored_value(index), _typed_value(l), _typed_value(r)]
			)
	return ""
...
static func error_has_size(current: Variant, expected: int) -> String:
	...
	return (
		"%s\n %s\n but was\n %s"
		% [_error("Expecting size:"), _colored_value(expected), _colored_value(current_size)]
	)
...
static func error_contains_exactly(current: Array, expected: Array) -> String:
	return (
		"%s\n %s\n but was\n %s"
		% [_error("Expecting exactly equal:"), _colored_value(expected), _colored_value(current)]
	)
...
static func format_chars(characters: PackedInt32Array, type: Color) -> PackedInt32Array:
	...
	var message := "[bgcolor=#%s][color=white]%s[/color][/bgcolor]" % [type.to_html(), ascii_text]
...
static func build_failure_message(
	failure: String, additional_failure_message: String, custom_failure_message: String
) -> String:

...
func setup_update_notification(control: Button) -> void:
	...
	if response.status() != 200:
		_test_reporter.println_message(
			"Information cannot be retrieved from GitHub!", GdUnitEditorColorTheme.state_failure
		)
		_test_reporter.println_message(
			"Error:  %s" % response.response(), GdUnitEditorColorTheme.state_failure
		)
		return
	var latest_version := update_client.extract_latest_version(response)
	if not latest_version.is_greater(GdUnit4Version.current()):
		_test_reporter.println_message(
			"GdUnit4 is up-to-date.", GdUnitEditorColorTheme.state_success
		)
		return

	_test_reporter.println_message(
		"A new update is available %s" % latest_version, GdUnitEditorColorTheme.state_warning
	)
	_test_reporter.println_message(
		"Open the GdUnit4 settings and check the update tab.", GdUnitEditorColorTheme.state_warning
	)
	...
	(
		tween
		. tween_property(control, "self_modulate", GdUnitEditorColorTheme.state_warning, .2)
...
func _on_gdunit_event(event: GdUnitEvent) -> void:
	match event.type():
		GdUnitEvent.SESSION_START:
			_test_reporter.test_session = GdUnitTestSession.new(
				GdUnitTestDiscoverGuard.instance().get_discovered_tests(), ""
			)
...
func _on_gdunit_client_connected(client_id: int) -> void:
	...
	_test_reporter.println_message(
		"GdUnit Test Client connected with id: %d" % client_id, GdUnitEditorColorTheme.folder_color
	)


func _on_gdunit_client_disconnected(client_id: int) -> void:
	_test_reporter.println_message(
		"GdUnit Test Client disconnected with id: %d" % client_id,
		GdUnitEditorColorTheme.folder_color
	)
